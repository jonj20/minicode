...
describe("extensions discovery", () => {
	...
	it("handles explicitly configured paths", async () => {
		...
		const result = await discoverAndLoadExtensions([customPath], tempDir, tempDir, undefined, {
			skipInternal: true,
		});
	...
	it("resolves dependencies from extension's own node_modules", async () => {
		...
		const result = await discoverAndLoadExtensions([extPath], tempDir, tempDir, undefined, {
			skipInternal: true,
		});
