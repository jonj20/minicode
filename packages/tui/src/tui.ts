	...
	private doRender(): void {
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				// Clamp over-wide lines to terminal width (instead of crashing).
				// Components should already truncate, but we keep this as a safety net.
				newLines[i] = sliceByColumn(line, 0, width, false);
				line = newLines[i]!;
			}
