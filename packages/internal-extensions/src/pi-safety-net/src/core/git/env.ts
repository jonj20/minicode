...
/** @internal - exported for test coverage */
export const GIT_SSH_ENV_NAMES: ReadonlySet<string> = new Set(["GIT_SSH_COMMAND", "GIT_SSH", "GIT_SSH_VARIANT"]);
...
export function parseGitContextAppendEnvAssignment(token: string): { name: string; value: string } | null {
...
export function hasConfigAffectingEnvAssignment(envAssignments?: ReadonlyMap<string, string>): boolean {
	if (!envAssignments) {
		return false;
	}
	for (const key of envAssignments.keys()) {
