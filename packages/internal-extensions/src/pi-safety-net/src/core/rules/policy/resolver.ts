...
import { assertBareRulebookName, GITHUB_RULEBOOK_PATH_RE, isGitHubRulebookSource, parseGitHubSource } from "./sources";
import type {
...
export async function discoverGitHubRepositoryRulebooks(source: string): Promise<DiscoveredRulebookSource[]> {
...
export function resolveLocalRulebook(spec: string, configDir: string, _options: RulesPolicyOptions): ResolvedRulebook {
...
async function fetchLockedGitHubRulebook(entry: GitHubRulebookLockEntry): Promise<ResolvedRulebook> {
	const rawResponse = await fetch(
...
async function resolveGitHubCommit(owner: string, repo: string, ref: string, source: string): Promise<string> {
