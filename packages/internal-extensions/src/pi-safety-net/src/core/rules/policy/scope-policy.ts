...
export function loadRulesPolicy(options: RulesPolicyOptions = {}): LoadedRulesPolicy {
	...
	const userPolicy = user.config
		? loadScopePolicy(user.config, paths.userLockPath, dirname(paths.userConfigPath), options, "user")
		: emptyScopePolicy();
	const projectPolicy = project.config
		? loadScopePolicy(project.config, paths.projectLockPath, dirname(paths.projectConfigPath), options, "project")
		: emptyScopePolicy();
...
function getLegacyRulesConfigErrors(paths: ReturnType<typeof getPolicyPaths>, options: RulesPolicyOptions): string[] {
	return Array.from(
		new Set([
			...getLegacyRulesConfigError(
				getLegacyUserRulesConfigPath(options),
				paths.userConfigPath,
				"~/.minicode/safety-net/config.json",
			),
			...getLegacyRulesConfigError(
...
function getLegacyRulesConfigError(legacyPath: string, configPath: string, migratedFrom: string): string[] {
	if (!existsSync(legacyPath)) return [];
	if (hasMigrationEvidence(configPath, migratedFrom)) return [];
	if (!legacyRulesConfigNeedsMigration(legacyPath)) return [];
	return [`legacy rules config location is no longer used; ask the user to run ${RULE_MIGRATE_COMMAND}`];
}
...
function hasMigrationEvidence(configPath: string, migratedFrom: string): boolean {
	...
	return config.rules.some((source) => getRulebookMigratedFrom(dirname(configPath), source) === migratedFrom);
}
...
function applyOverrides(rules: CustomRule[], overrides: Record<string, RuleOverride>): CustomRule[] {
...
function getUnknownOverrideErrors(overrides: Record<string, RuleOverride>, knownRuleIds: Set<string>): string[] {
