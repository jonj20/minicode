...
export function getProjectRulesDir(cwd?: string): string {
	// Use .minicode/safety-net instead of .minicode/safety-net/rules
	return resolve(cwd ?? process.cwd(), ".minicode", "safety-net", RULES_SUBDIR);
}
...
export function getUserRulesDir(options?: RulesPolicyOptions): string {
	return (
		...
		(options?.userConfigPath ? dirname(options.userConfigPath) : join(getUserSafetyNetHome(), RULES_SUBDIR))
	);
}
...
function getUserSafetyNetHome(): string {
	...
	// Use minicode config directory instead of .minicode/safety-net
	return join(homedir(), ".minicode", "safety-net");
}
...
export function getRulebookCachePath(entry: RulebookLockEntry, options?: RulesPolicyOptions): string {
