...
export async function syncRulesConfig(options: SyncRulesConfigOptions = {}): Promise<SyncRulesConfigResult> {
...
function addRuleCount(entry: RulebookLockEntry, ruleCountsBySpec: Map<string, number>): RulebookLockEntryWithStats {
	return {
		...entry,
...
function writeCache(content: string, entry: RulebookLockEntry, configDir: string, options: RulesPolicyOptions): void {
...
function pruneUnreferencedRulebookCaches(
	...
	const keep = new Set(
		entries.map((entry) => dirname(getRulebookCachePath(entry, { ...options, cacheConfigDir: configDir }))),
	);
