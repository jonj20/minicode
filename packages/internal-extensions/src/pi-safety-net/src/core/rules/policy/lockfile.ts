...
export function readLockfile(path: string): { lock: RulesLockfile | null; errors: string[] } {
	...
	try {
		...
		return {
			lock: null,
			errors: [`malformed lockfile ${path}: ${error instanceof Error ? error.message : String(error)}`],
		};
	}
}

function parseLockEntry(entry: unknown, prefix: string): { entry: RulebookLockEntry | null; errors: string[] } {
...
function validateRequiredString(candidate: Record<string, unknown>, prefix: string, field: string): string[] {
