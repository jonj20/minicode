...
function noRulebookMatch(match: string, nameMatches: string[]): { ok: false; result: SyncRulesConfigResult } {
	return {
		ok: false,
		result: {
			ok: false,
...
function getConfiguredGitHubSource(spec: string): { owner: string; repo: string; ref: string } | null {
	try {
