...
function getCommandSubstitution(tokens: ParseEntry[], index: number) {
	...
	return {
		...
		attachedSuffix,
		shouldKeepCurrent:
			attachedSuffix !== null && !_isRedirectOp(tokens[index - 1]) && !isOperatorToken(tokens[index - 1]),
	};
}
...
function extractArithmeticNestedCommand(
	...
	const { innerSegments: nestedSegments, endIndex } = extractCommandSubstitution(tokens as ParseEntry[], startIndex);
	for (const seg of nestedSegments) {
		innerSegments.push(seg);
	}
...
function _pushInlineSubstitutionSegmentInfos(segments: ShellCommandSegmentInfo[], token: string): void {
	const inlineSegments = extractInlineCommandSubstitutions(token);
	for (const seg of inlineSegments) {
		segments.push({ tokens: seg, hasDynamicSubstitution: false });
	}
...
function _readAnsiCString(command: string, startIndex: number): { value: string; endIndex: number } | null {
	let value = "";
