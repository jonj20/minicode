...
export class ZellijModalFrame {
	...
	private renderTitleBar(width: number, palette: ZellijColorPalette): string {
		return this.renderBorderLine(
			width,
			palette,
			this.borders.topLeft,
...
export class ZellijSettingsModal implements ZellijModalContentRenderer {
	...
	constructor(options: SettingsModalOptions, theme: Theme) {
		...
		this.tabLists = this.showTabs
			? options.tabs!.map((tab) => this.createSettingsList(tab.settings, () => this.options.onClose()))
			: [];
...
function truncateWithNoOpGuard(text: string, maxWidth: number, compute: () => string): string {
