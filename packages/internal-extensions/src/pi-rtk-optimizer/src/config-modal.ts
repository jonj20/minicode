...
import { toOnOff } from "./boolean-format";
import type { RtkIntegrationController } from "./command-register";
import {
...
import { ZellijModal, ZellijSettingsModal } from "./zellij-modal";
...
function buildSettingItems(config: RtkIntegrationConfig): SettingItem[] {
	return [
		...
		},
		{
			...
			description:
				"If on, read results under the global Pi skills directory (default: ~/.minicode/agent/skills, respects PI_CODING_AGENT_DIR), ~/.agents/skills, .minicode/skills, and ancestor .agents/skills skip read compaction",
