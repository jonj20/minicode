import { type ExtensionAPI, ensureTool, getToolPath } from "@earendil-works/pi-coding-agent";

export type RtkExecutableResolverName = "where" | "which" | "managed";
...
export async function resolveRtkExecutable(
	...
	// First check for managed rtk binary in the bin directory
	let managedPath = getToolPath("rtk");
	if (!managedPath) {
		// Not found locally — trigger download (silent, non-blocking)
		const downloaded = await ensureTool("rtk", true);
		if (downloaded) {
			managedPath = downloaded;
		}
	}
	if (managedPath) {
		return {
