...
import { handleForkRestore, handleTreeRestore, registerCommands } from "./commands.ts";
import {
	createCheckpoint,
...
import { clearStatus, updateStatus } from "./ui.ts";
...
export default function (pi: ExtensionAPI) {
	...
	async function initSession(ctx: any): Promise<void> {
		...
		if (state.repoRoot && state.sessionId) {
			...
			pruneOldSessions(root, sid)
				.then((pruned) => {
					if (pruned > 0) {
						// Reload cache after prune
						loadAllCheckpoints(root, sid)
							.then((remaining) => {
	...
	pi.on("turn_end", async (_event, ctx) => {
		...
		// Auto-prune
		try {
			const pruned = await pruneCheckpoints(state.repoRoot, state.sessionId, DEFAULT_MAX_CHECKPOINTS);
			if (pruned > 0) {
