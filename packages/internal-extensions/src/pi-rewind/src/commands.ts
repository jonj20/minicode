...
import { createCheckpoint, diffCheckpoints, git, restoreCheckpoint } from "./core.ts";
...
function formatCheckpointLabel(cp: CheckpointData, index: number, _state: RewindState, currentBranch?: string): string {
	...
	const branchTag =
		cp.branch && currentBranch && cp.branch !== currentBranch
			? ` ⚠️ ${cp.branch}`
			: cp.branch
				? ` [${cp.branch}]`
				: "";
...
async function runRewindFlow(
	...
	const checkpoints = [...state.checkpoints.values()].sort((a, b) => b.timestamp - a.timestamp).slice(0, MAX_DISPLAY);
	...
	const what = mode === "all" ? "files + conversation" : mode === "files" ? "files" : "conversation";
...
export function registerCommands(pi: ExtensionAPI, state: RewindState): void {
	...
	pi.registerShortcut("escape escape", {
		...
		handler: async (ctx) => {
			...
			const checkpoints = [...state.checkpoints.values()].sort((a, b) => b.timestamp - a.timestamp).slice(0, 25);
