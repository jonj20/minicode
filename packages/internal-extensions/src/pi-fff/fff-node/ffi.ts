...
import { close, DataType, isNullPointer, type JsExternal, load, open, restorePointer, wrapPointer } from "ffi-rs";
...
function parseGrepResult(rawPtr: JsExternal): Result<GrepResult> {
	...
	for (let i = 0; i < count; i++) {
		const rawMatch = callAccessor<FffGrepMatchRaw>("fff_grep_result_get_match", handlePtr, i, FFF_GREP_MATCH_STRUCT);
...
function parseSearchResult(rawPtr: JsExternal): Result<SearchResult> {
	...
	for (let i = 0; i < count; i++) {
		const rawItem = callAccessor<FffFileItemRaw>("fff_search_result_get_item", handlePtr, i, FFF_FILE_ITEM_STRUCT);
		items.push(readFileItemFromRaw(rawItem));

		const rawScore = callAccessor<FffScoreRaw>("fff_search_result_get_score", handlePtr, i, FFF_SCORE_STRUCT);
...
function parseDirSearchResult(rawPtr: JsExternal): Result<DirSearchResult> {
	...
	for (let i = 0; i < count; i++) {
		const rawItem = callAccessor<FffDirItemRaw>("fff_dir_search_result_get_item", handlePtr, i, FFF_DIR_ITEM_STRUCT);
		items.push(readDirItemFromRaw(rawItem));

		const rawScore = callAccessor<FffScoreRaw>("fff_dir_search_result_get_score", handlePtr, i, FFF_SCORE_STRUCT);
		scores.push(readScoreFromRaw(rawScore));
	}
...
function parseMixedSearchResult(rawPtr: JsExternal): Result<MixedSearchResult> {
	...
	for (let i = 0; i < count; i++) {
		...
		const rawScore = callAccessor<FffScoreRaw>("fff_mixed_search_result_get_score", handlePtr, i, FFF_SCORE_STRUCT);
		scores.push(readScoreFromRaw(rawScore));
	}
 ...
 */
export function ffiTrackQuery(handle: NativeHandle, query: string, filePath: string): Result<boolean> {
 ...
 */
export function ffiGetHistoricalQuery(handle: NativeHandle, offset: number): Result<string | null> {
 ...
 */
export function ffiHealthCheck(handle: NativeHandle | null, testPath: string): Result<unknown> {
	if (handle === null) {
		// Use U64(0) as a null pointer since ffi-rs rejects null for External params
		return callJsonResult<unknown>("fff_health_check", [DataType.U64, DataType.String], [0, testPath]);
	}

	return callJsonResult<unknown>("fff_health_check", [DataType.External, DataType.String], [handle, testPath]);
}
