...
import { createModuleReportTool, createReadEnclosingTool, createReadSymbolTool } from "./tools/module-report.js";
...
const DEBUG_LOG_DIR = path.join(os.homedir(), ".minicode", ".pi-lens");
...
const LSP_TOOLCALL_NAV_TOUCH_BUDGET_MS = Math.max(
	0,
	Number.parseInt(
		process.env.PI_LENS_TOOLCALL_NAV_TOUCH_MS ?? process.env.PI_LENS_LSP_NAV_CLIENT_WAIT_MS ?? "1500",
		10,
	) || 1500,
);
...
function getEffectiveReadLimit(filePath: string | undefined, readInput: ReadToolInput | undefined): number | undefined {
...
function countOldTextMatches(filePath: string, oldText: string, cachedNormalizedContent?: string): number {
	try {
		const content = cachedNormalizedContent ?? normalizeOldTextForMatch(nodeFs.readFileSync(filePath, "utf-8"));
...
export default function (pi: ExtensionAPI) {
	...
	function updateLspStatus(setStatus: (id: string, text: string | undefined) => void, theme: LspStatusTheme) {
		try {
	...
	pi.registerFlag("no-lsp", {
		description: "Disable unified LSP diagnostics and use language-specific fallbacks (for example pyright)",
		type: "boolean",
		default: false,
	});
	...
	pi.registerFlag("extended-tools", {
		description:
			"Enable extended pi-lens tools (lens_diagnostics, ast_grep_replace, ast_grep_outline, ast_grep_dump). Increases system prompt tokens by ~400.",
		type: "boolean",
		default: false,
	});
	...
	function getLensFlag(name: string): boolean | string | undefined {
		const cliValue = globalConfigOnlyFlags.has(name) ? undefined : pi.getFlag(name);
	...
	let contextInjectionEnabled = process.env.PI_LENS_NO_CONTEXT_INJECTION !== "1" && !getLensFlag("no-lens-context");
	...
	const extendedToolsEnabled = !!getLensFlag("extended-tools");
	const coreTools = [
		...
		createReadSymbolTool(
			...
			(filePath, symbol) =>
				runtime.readGuard.recordSymbolRead(filePath, symbol, runtime.turnIndex, runtime.peekWriteIndex()),
		),
	...
	// Extended tools: lens_diagnostics, lsp_diagnostics, ast_grep_replace, ast_grep_outline, ast_grep_dump
	// Enabled via --extended-tools flag or extendedTools.enabled in global config.
	const extendedTools = extendedToolsEnabled
		? [
		...
		: [];
	for (const tool of [...coreTools, ...extendedTools]) {
		try {
	...
	pi.on("resources_discover", async (_event, _ctx) => {
		// Resolve skills directory relative to the pi-lens extension location.
		let skillsDir: string;

		try {
			// In Bun binary, import.meta.url points to a virtual filesystem (B:/%7EBUN/root/...)
			// which doesn't contain the skills. Extract embedded skills to global directory.
			const isBunBinary = typeof globalThis.Bun !== "undefined";
			if (isBunBinary) {
				const globalSkillsDir = path.join(os.homedir(), ".minicode", "skills");
				const skillsMarker = path.join(globalSkillsDir, ".pi-lens-extracted");
				if (!nodeFs.existsSync(skillsMarker)) {
					// Extract embedded skills
					try {
						const { EMBEDDED_INTERNAL_EXTENSIONS } = await import("@earendil-works/pi-coding-agent/embedded");
						const skillPrefix = "internal-extensions/src/pi-lens/skills/";
						for (const [key, content] of Object.entries(EMBEDDED_INTERNAL_EXTENSIONS)) {
							if (key.startsWith(skillPrefix) && key.endsWith("SKILL.md")) {
								const relPath = key.slice(skillPrefix.length);
								...
								nodeFs.writeFileSync(path.join(skillDir, "SKILL.md"), content);
							}
						}
						nodeFs.mkdirSync(globalSkillsDir, { recursive: true });
						nodeFs.writeFileSync(skillsMarker, new Date().toISOString());
					} catch {
						return { skillPaths: [] };
					}
				}
				return { skillPaths: [globalSkillsDir] };
			}

			// 1. Try to find skills relative to this extension's source location
			// The extension is at packages/internal-extensions/src/pi-lens/index.ts
			// Skills are at packages/internal-extensions/src/pi-lens/skills/
			const currentDir = path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, "$1"));
			skillsDir = path.join(currentDir, "skills");

			// 2. If not found, try in .minicode/skills/ (project-level)
			if (!nodeFs.existsSync(skillsDir)) {
				const projectSkillsDir = path.join(process.cwd(), ".minicode", "skills");
				if (nodeFs.existsSync(projectSkillsDir)) {
					skillsDir = projectSkillsDir;
				}
			}

			// 3. Final fallback: don't return any skills path
			if (!nodeFs.existsSync(skillsDir)) {
				return { skillPaths: [] };
			}
		} catch {
	...
	pi.on("session_start", async (event, ctx) => {
		try {
			...
			await handleSessionStart({
				...
				goClient,
				rustClient,
				ensureTool: async (name: string) => (await import("./clients/installer/index.js")).ensureTool(name),
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (getLensFlag("lens-guard") && isGitCommitOrPushAttempt(toolName, event.input)) {
		...
		if (!getLensFlag("no-lsp")) {
			try {
				const configCwd = filePath ? path.dirname(filePath) : (ctx.cwd ?? runtime.projectRoot ?? process.cwd());
		...
		const shouldWarmReadLsp = toolName === "read" && lspAutoTouchEligible && runtime.shouldWarmLspOnRead(filePath);
		const shouldAutoTouch =
			(toolName === "write" || toolName === "edit" || toolName === "lsp_navigation" || shouldWarmReadLsp) &&
			!getLensFlag("no-lsp") &&
		...
		if (shouldAutoTouch) {
			try {
				...
				const maxClientWaitMs =
					toolName === "lsp_navigation" ? LSP_TOOLCALL_NAV_TOUCH_BUDGET_MS : LSP_TOOLCALL_TOUCH_BUDGET_MS;
				if (toolName === "read") {
		...
		if (
			...
			const totalLines =
				effectiveReadLimit != null && requestedReadLimit == null ? effectiveReadLimit : countFileLines(filePath);
			try {
	...
	(pi as any).on("tool_result", async (event: any, ctx: any) => {
		...
		if (rtToolName === "edit" || rtToolName === "write") {
			logLatency({
				...
				filePath: (event as { input?: { path?: string } })?.input?.path ?? "<unknown>",
				durationMs: 0,
	...
	(pi as any).on(
		"context",
		async (event: { messages?: Array<{ role: string; content: unknown }> } | unknown, ctx: { cwd?: string }) => {
