...
export function recordFromDispatchDiagnostic(diagnostic: Diagnostic, cwd: string): ActionableWarningRecord | undefined {
	if (diagnostic.semantic !== "warning" || diagnostic.severity !== "warning") return undefined;
...
function recordFromLspDiagnostic(diag: LSPDiagnostic, filePath: string, cwd: string): ActionableWarningRecord {
...
export async function buildActionableWarningsReport(args: {
	...
	if (args.includeLspCodeActions) {
		for (const file of args.files) {
			...
			const currentContent = fs.existsSync(filePath) ? fs.readFileSync(filePath, "utf-8") : undefined;
			const contentHash =
				currentContent !== undefined ? createHash("sha256").update(currentContent).digest("hex") : undefined;
			const cached =
				contentHash !== undefined ? lspService.getLastKnownDiagnostics(filePath, contentHash) : undefined;
			if (cached !== undefined) {
				diags = cached;
			} else {
				try {
 ...
 */
export function appendActionableWarningsHistory(cwd: string, report: ActionableWarningsReport): void {
...
export async function applyConservativeActionableWarningFixes(args: {
	...
	for (const file of args.report.files) {
		...
		for (const warning of file.warnings) {
			...
			try {
				const content = fs.existsSync(warning.filePath) ? fs.readFileSync(warning.filePath, "utf-8") : undefined;
...
export function formatActionableWarningsAdvisory(report: ActionableWarningsReport): string | undefined {
