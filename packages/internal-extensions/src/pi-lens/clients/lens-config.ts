...
export interface PiLensGlobalConfig {
	/**
	 * Whether pi-lens is enabled. Defaults false (off) — set true to activate.
	 * CLI --no-lens overrides this to disable for a single session.
	 */
	enabled?: boolean;
	/**
...
export function getPiLensGlobalConfigPath(homeDir = os.homedir()): string {
	...
	return path.join(homeDir, ".minicode", ".pi-lens", "config.json");
}

export function loadPiLensGlobalConfig(configPath = getPiLensGlobalConfigPath()): PiLensGlobalConfig | undefined {
	try {
		...
		const dispatch =
			dispatchRaw && typeof dispatchRaw === "object" ? (dispatchRaw as Record<string, unknown>) : undefined;
		const widgetRaw = raw.widget;
		const widget = widgetRaw && typeof widgetRaw === "object" ? (widgetRaw as Record<string, unknown>) : undefined;
		const formatRaw = raw.format;
		const format = formatRaw && typeof formatRaw === "object" ? (formatRaw as Record<string, unknown>) : undefined;
		...
		const actionableWarningsAutoFix =
			actionableWarningsAutoFixRaw && typeof actionableWarningsAutoFixRaw === "object"
		...
		const contextInjection =
			contextInjectionRaw && typeof contextInjectionRaw === "object"
				...
				: undefined;
		const formatMode = format?.mode === "immediate" || format?.mode === "deferred" ? format.mode : undefined;
		...
		return {
			...
			actionableWarnings: actionableWarnings
				? {
						enabled: typeof actionableWarnings.enabled === "boolean" ? actionableWarnings.enabled : undefined,
						includeLspCodeActions:
							typeof actionableWarnings.includeLspCodeActions === "boolean"
								? actionableWarnings.includeLspCodeActions
								: undefined,
						deltaOnly:
							typeof actionableWarnings.deltaOnly === "boolean" ? actionableWarnings.deltaOnly : undefined,
			...
			contextInjection: contextInjection
				? {
						enabled: typeof contextInjection.enabled === "boolean" ? contextInjection.enabled : undefined,
					}
				: undefined,
		};
	} catch {
...
export function getGlobalWidgetDefaultVisible(configPath?: string): boolean {
	return loadPiLensGlobalConfig(configPath)?.widget?.visible === true;
}

export function getGlobalAutoformatEnabled(configPath?: string): boolean {
	return loadPiLensGlobalConfig(configPath)?.format?.enabled === true;
}
...
export function getGlobalContextInjectionEnabled(configPath?: string): boolean {
	return loadPiLensGlobalConfig(configPath)?.contextInjection?.enabled === true;
}

export function getGlobalLensEnabled(configPath?: string): boolean {
	return loadPiLensGlobalConfig(configPath)?.enabled !== false;
}
...
export function resolvePiLensFlag(
	...
	if (name === "no-lens") {
		// Default: ON. Only disabled when config explicitly sets enabled: false.
		return config?.enabled === false;
	}
	if (name === "no-autoformat") {
		// Default: OFF (minimal mode). Only enabled when config explicitly sets format.enabled: true.
		return config?.format?.enabled !== true;
	}
	...
	if (name === "no-lens-context") {
		// Default: OFF (minimal mode). Only enabled when config explicitly sets contextInjection.enabled: true.
		return config?.contextInjection?.enabled !== true;
	}
	return value;
}
