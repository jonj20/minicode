...
function isTextDocumentEdit(value: unknown): value is TextDocumentEdit {
	return (
		typeof value === "object" &&
		value !== null &&
		typeof (value as { textDocument?: { uri?: unknown } }).textDocument?.uri === "string" &&
...
export function summarizeWorkspaceEdit(
	...
	for (const change of edit.documentChanges ?? []) {
		if (typeof change !== "object" || change === null || !("kind" in change)) continue;
		const kind = (change as { kind?: unknown }).kind;
		if (kind === "create" && typeof (change as CreateFileOp).uri === "string") {
			lines.push(`Create ${relativeToCwd(uriToPath((change as CreateFileOp).uri), cwd)}`);
		} else if (
			kind === "rename" &&
		...
		} else if (kind === "delete" && typeof (change as DeleteFileOp).uri === "string") {
			lines.push(`Delete ${relativeToCwd(uriToPath((change as DeleteFileOp).uri), cwd)}`);
		}
	}
	return lines;
}
	...
	try {
		...
		for (const change of edit.documentChanges ?? []) {
			...
			if (kind === "create" && typeof (change as CreateFileOp).uri === "string") {
				...
				await fs.writeFile(filePath, "", { flag: "wx" }).catch(async (err: unknown) => {
