...
const DIRECT_LSP_NEGATIVE_TTL_MS = Math.max(
	30_000,
	Number.parseInt(process.env.PI_LENS_DIRECT_LSP_NEGATIVE_TTL_MS ?? "600000", 10) || 600_000,
);
...
function piAgentExtensionsRootKey(file: string): string | undefined {
	...
	const marker = "/.minicode/agent/extensions";
...
function createInteractiveServer(spec: InteractiveServerSpec): LSPServerInfo {
	return {
		...
		root: spec.root,
		availabilityKey: typeof spec.command === "string" && isSimpleCommand(spec.command) ? spec.command : undefined,
		async spawn(root) {
			...
			try {
				...
				const initialization =
					typeof spec.initialization === "function" ? spec.initialization(root) : spec.initialization;
...
export function PriorityRoot(markerGroups: string[][], excludePatterns?: string[], stopDir?: string): RootFunction {
...
export const FileDirRoot: RootFunction = async (file: string) => path.resolve(path.dirname(file));

export function RootWithFallback(primary: RootFunction, fallback: RootFunction = FileDirRoot): RootFunction {
	return async (file: string): Promise<string | undefined> => {
...
export function WorkspacePriorityRoot(markerGroups: string[][], excludePatterns?: string[]): RootFunction {
...
async function markerExists(dir: string, pattern: string): Promise<boolean> {
	...
	const targetDir = parentPattern ? path.join(dir, ...parentPattern.split("/").filter(Boolean)) : dir;
	try {
 ...
 */
export function NearestRoot(includePatterns: string[], excludePatterns?: string[], stopDir?: string): RootFunction {
	...
	return async (file: string): Promise<string | undefined> => {
		...
		const promise = (async (): Promise<string | undefined> => {
			...
			while (true) {
				if (stop !== fsRoot && currentDir.startsWith(stop + path.sep) === false && currentDir !== stop) {
					break;
				}
...
export async function tryGoInstallGopls(): Promise<boolean> {
	...
	const result = await safeSpawnAsync(isWindows ? "go.exe" : "go", ["install", "golang.org/x/tools/gopls@latest"], {
		timeout: 180000,
...
export async function tryDotnetToolInstall(tool: string): Promise<boolean> {
	...
	const result = await safeSpawnAsync("dotnet", ["tool", "install", "--tool-path", PI_LENS_BIN_DIR, tool], {
		timeout: 180000,
		ignoreAmbientSignal: true,
	});
	...
	const updateResult = await safeSpawnAsync("dotnet", ["tool", "update", "--tool-path", PI_LENS_BIN_DIR, tool], {
		timeout: 180000,
		ignoreAmbientSignal: true,
	});
 ...
 */
async function findTsserverPath(root: string, allowInstall: boolean | undefined): Promise<string | undefined> {
...
function binExeVariants(dir: string, tool: string): string[] {
	return process.platform === "win32" ? [path.join(dir, `${tool}.exe`), path.join(dir, tool)] : [path.join(dir, tool)];
}
...
export async function tryGemInstall(gem: string): Promise<boolean> {
	...
	const binDir = join(homedir(), ".minicode", ".pi-lens", "bin");
	const { mkdir } = await import("node:fs/promises");
	await mkdir(binDir, { recursive: true });

	const result = await safeSpawnAsync("gem", ["install", gem, "--bindir", binDir, "--no-document"], {
		timeout: 180000,
		ignoreAmbientSignal: true,
	});
...
export async function detectPythonVenv(root: string): Promise<string | undefined> {
	...
	for (const venv of candidates) {
		const pythonPath = isWin ? path.join(venv, "Scripts", "python.exe") : path.join(venv, "bin", "python");
		try {
 ...
 * but stop at `extensionRootKey` so we never escape the .minicode/agent/extensions
 * boundary into a higher-up project (e.g. ~/.minicode/agent/package.json which
 ...
 */
async function findExtensionBoundedRoot(file: string, extensionRootKey: string): Promise<string | undefined> {
	const startDir = path.resolve(path.dirname(file));
	let currentDir = startDir;
	while (true) {
 ...
 * (i.e. `.minicode/agent/`) holds any TypeScript project marker. This narrowly
 * detects the #123 scenario — pi itself installs a package.json at
 * `~/.minicode/agent/` and the user's extension has none of its own — without
...
const TypeScriptRoot: RootFunction = DenoExcludeRoot(async (file) => {
	...
	if (extensionRootKey) {
		// Bounded walk so we never adopt a parent (e.g. ~/.minicode/agent/) as the
		// LSP root.
		...
		// package.json at ~/.minicode/agent/ (the #123 setup), the previous code
...
export const PythonServer: LSPServerInfo = {
	...
	async spawn(root, options) {
		...
		const managedCandidates = isWindows
			? [path.join(binDir, "pyright-langserver.cmd"), path.join(binDir, "pyright-langserver"), "pyright-langserver"]
...
export const CppServer: LSPServerInfo = {
	...
	root: RootWithFallback(createRootDetector(["compile_commands.json", ".clangd", "CMakeLists.txt", "Makefile"])),
	spawn(root, options) {
...
export const YamlServer: LSPServerInfo = {
	...
	root: RootWithFallback(PriorityRoot([[".yamllint", "yamllint.yml", "yamllint.yaml", "pyproject.toml"], [".git"]])),
	spawn(root, options) {
		return resolveAndLaunch(
			{
...
export const JsonServer: LSPServerInfo = {
	...
	root: RootWithFallback(WorkspacePriorityRoot([["package.json", "tsconfig.json", "jsconfig.json"], [".git"]])),
	spawn(root, options) {
		return resolveAndLaunch(
			{
...
export const VueServer: LSPServerInfo = {
	...
	async spawn(root, options) {
		...
		return {
			...
			source: proc.source,
			initialization: tsserverPath ? { typescript: { tsdk: path.dirname(tsserverPath) } } : undefined,
		};
	},
};
...
export const CssServer: LSPServerInfo = {
	...
	root: RootWithFallback(
		IgnoreHomeRoot(PriorityRoot([["package.json", "postcss.config.js", "tailwind.config.js", "vite.config.ts"]])),
	),
	spawn(root, options) {
		return resolveAndLaunch(
			{
...
const OPENGREP_EXTENSIONS: readonly string[] = Array.from(
	new Set(OPENGREP_KINDS.flatMap((k) => (KIND_EXTENSIONS as Record<string, readonly string[]>)[k] ?? [])),
);
...
const AST_GREP_EXTENSIONS: readonly string[] = Array.from(
	new Set(AST_GREP_KINDS.flatMap((k) => (KIND_EXTENSIONS as Record<string, readonly string[]>)[k] ?? [])),
);
...
const TYPOS_EXTENSIONS: readonly string[] = Array.from(new Set([...OPENGREP_EXTENSIONS, ...KIND_EXTENSIONS.markdown]));
