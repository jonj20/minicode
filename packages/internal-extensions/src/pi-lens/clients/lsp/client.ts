...
export interface LSPClientInfo {
	...
	waitForDiagnostics(filePath: string, timeoutMs?: number, options?: { minVersion?: number }): Promise<void>;
	 ...
	 * call (and only then).
	 */
	executeCommand(command: string, args?: unknown[]): Promise<{ executed: boolean; result?: unknown; reason?: string }>;
...
const LSP_CRASH_CODES = new Set(["ERR_STREAM_DESTROYED", "ERR_STREAM_WRITE_AFTER_END", "EPIPE", "ECONNRESET"]);
...
function mergeDiagnosticLists(push: LSPDiagnostic[] | undefined, pull: LSPDiagnostic[] | undefined): LSPDiagnostic[] {
...
function getMergedDiagnosticsForPath(state: LSPClientState, normalizedPath: string): LSPDiagnostic[] {
...
function clearDiagnosticsForPath(state: LSPClientState, normalizedPath: string): void {
	const legacy = state as unknown as {
		diagnostics?: Map<string, LSPDiagnostic[]>;
...
export function applyDynamicCapabilities(state: LSPClientState): void {
	...
	const hasDynamicPull =
		registeredMethods.has("textDocument/diagnostic") || registeredMethods.has("workspace/diagnostic");
...
function setupIncomingHandlers(state: LSPClientState, initialization: Record<string, unknown> | undefined): void {
	...
	state.connection.onRequest(
		...
		async (params: { edit?: { changes?: unknown; documentChanges?: unknown } }) => {
			...
			try {
				await applyWorkspaceEdit(params.edit as Parameters<typeof applyWorkspaceEdit>[0], state.root);
				return { applied: true };
			} catch (err) {
				return {
					applied: false,
	...
	state.connection.onRequest("workspace/configuration", async () => [initialization ?? {}]);
 ...
 */
type PullDiagnosticsOutcome = { status: "found"; count: number } | { status: "clean" } | { status: "unavailable" };
...
export async function clientWaitForDiagnostics(
	...
	const hasFreshDiagnostics = (): boolean => minVersion === undefined || state.diagnosticsVersion > minVersion;
	...
	if (hasFreshDiagnostics() && !isVersionStale() && getMergedDiagnosticsForPath(state, normalizedPath).length > 0) {
		return;
	}
...
export async function handleNotifyChange(state: LSPClientState, filePath: string, content: string): Promise<void> {
	if (!isClientAlive(state)) return;
	const uri = pathToFileURL(filePath).href;
	const normalizedPath = normalizeMapKey(filePath);
...
export async function clientShutdown(state: LSPClientState, options: LSPShutdownOptions = {}): Promise<void> {
	state.isConnected = false;
	state.isDestroyed = true;
...
export async function navRequest<T>(
	...
	const requestVersion = normalizedPath !== undefined ? state.documentVersions.get(normalizedPath) : undefined;
	const result = (await withTimeout(safeSendRequest<T>(state.connection, method, params, signal), timeoutMs).catch(
		(err: unknown) => {
...
async function resolveCodeActionBestEffort(state: LSPClientState, action: LSPCodeAction): Promise<LSPCodeAction> {
...
export async function createLSPClient(options: {
	...
	state.positionEncoding = negotiatePositionEncoding((initResult as { capabilities?: unknown })?.capabilities);
	...
	return {
		...
		/** True if the server process has exited or been killed. */
		processExited: () =>
			lspProcess.process.exitCode !== null || (lspProcess.process as { killed?: boolean }).killed === true,
		...
		getAllDiagnostics() {
			...
			const keys = new Set([...state.pushDiagnostics.keys(), ...state.documentPullDiagnostics.keys()]);
		...
		getTrackedDiagnosticPaths() {
			return [...new Set([...state.pushDiagnostics.keys(), ...state.documentPullDiagnostics.keys()])];
		},
		...
		async codeAction(filePath, line, character, endLine, endCharacter) {
			...
			const actions = result.filter(
				(item): item is LSPCodeAction => typeof item === "object" && item !== null && "title" in item,
			);
		...
		async incomingCalls(item) {
			const result = await navRequest<LSPCallHierarchyIncomingCall[]>(state, "callHierarchy/incomingCalls", {
				item,
			});
			return result ?? [];
		},
		...
		async outgoingCalls(item) {
			const result = await navRequest<LSPCallHierarchyOutgoingCall[]>(state, "callHierarchy/outgoingCalls", {
				item,
			});
			return result ?? [];
		},
...
async function safeSendNotification(connection: MessageConnection, method: string, params: unknown): Promise<void> {
	try {
...
function detectWorkspaceDiagnosticsSupport(initResult: unknown): LSPWorkspaceDiagnosticsSupport {
	...
	if (typeof diagnosticProvider === "object") {
		return {
			advertised: true,
			mode: "pull",
			workspaceDiagnostics: (diagnosticProvider as { workspaceDiagnostics?: unknown }).workspaceDiagnostics === true,
