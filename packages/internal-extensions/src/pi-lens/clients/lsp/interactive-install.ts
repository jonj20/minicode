 ...
 */
async function saveChoice(cwd: string, toolId: string, choice: "yes" | "no" | "auto"): Promise<void> {
...
function isAutoInstallEnabled(): boolean {
	...
	return process.env.PI_LENS_AUTO_INSTALL === "1" || process.argv.includes("--auto-install");
}
 ...
 */
export function _parseStaticInstallCommandForTest(command: string): [string, string[]] | undefined {
 ...
 */
export async function promptForInstall(language: string, cwd: string): Promise<boolean> {
 ...
 */
export function getInstallStrategy(language: string): InstallStrategy | undefined {
