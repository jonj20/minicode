...
import { getServerInitOverride, getServersForFileWithConfig } from "./config.js";
...
import { isDirectLspCommandTemporarilyUnavailable, LSP_SERVERS } from "./server.js";
...
const NAV_CLIENT_WAIT_TIMEOUT_MS = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_LSP_NAV_CLIENT_WAIT_MS ?? "1500", 10) || 1500,
);
const TOUCH_DEBOUNCE_MS = Math.max(0, Number.parseInt(process.env.PI_LENS_LSP_TOUCH_DEBOUNCE_MS ?? "1500", 10) || 1500);
...
const DIAGNOSTICS_SEMANTIC_SETTLE_THRESHOLD_MS = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_LSP_DIAGNOSTICS_SEMANTIC_THRESHOLD_MS ?? "250", 10) || 250,
);
const DIAGNOSTICS_SEMANTIC_SETTLE_WAIT_MS = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_LSP_DIAGNOSTICS_SEMANTIC_SETTLE_MS ?? "400", 10) || 400,
);
...
const EARLY_UNBLOCK_GRACE_MS = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_LSP_EARLY_UNBLOCK_GRACE_MS ?? "400", 10) || 400,
);
...
function mergeLspDiagnostics(
	...
	for (const diagnostic of diagnostics) {
		const key = [diagnostic.range.start.line, diagnostic.range.start.character, diagnostic.message].join(":");
...
export class LSPService {
	 ...
	 */
	private shouldSkipNotify(filePath: string, content: string, clientScope: LSPTouchClientScope): boolean {
	...
	private markTouched(filePath: string, content: string, clientScope: LSPTouchClientScope): void {
		const key = `${normalizeMapKey(filePath)}:${clientScope}`;
		const now = Date.now();
	...
	private activeClientsForCwd(
		...
		for (const [key, client] of this.state.clients) {
			...
			const sameOrNested =
				root === normalizedCwd || root.startsWith(`${normalizedCwd}/`) || normalizedCwd.startsWith(`${root}/`);
	...
	async getClientForFile(
		...
		const effectiveMaxWaitMs =
			hardCapMs !== undefined ? (serverBaseMs > 0 ? Math.min(serverBaseMs, hardCapMs) : hardCapMs) : serverBaseMs;
		...
		const waitResult = await Promise.race<SpawnedServer | undefined | typeof timeoutSentinel>([
			withBudget(),
	 ...
	 */
	async getClientsForFile(filePath: string): Promise<{ clients: SpawnedServer[]; serverCountAttempted: number }> {
	 ...
	 */
	async getAuxiliaryClientsForFile(filePath: string, enabledIds: ReadonlySet<string>): Promise<SpawnedServer[]> {
	...
	private async ensureClientForServer(filePath: string, server: LSPServerInfo): Promise<SpawnedServer | undefined> {
		const root = await server.root(filePath);
	...
	async touchFile(
		...
		const clientScope: LSPTouchClientScope = options.clientScope ?? (diagnosticsMode === "full" ? "all" : "primary");
		...
		if (!notifySkipped) {
			...
			const wrote = await withDeadline(
				Promise.all(
					spawned.map((entry) => entry.client.notify.open(filePath, content, languageId, undefined, silent)),
				),
		...
		if (diagnosticsMode !== "none") {
			...
			if (envWait !== undefined) {
				...
				timeoutFor = () => envWait;
			} else if ((!useAllClients && spawned.length === 1) || clientScope === "with-auxiliary") {
	...
	async getDiagnostics(
		...
		const perServer = await raceToCompletion(
			clientWaits,
			(results) => results.some((r) => r.diagnosticCount > 0 || getStrategy(r.serverId).seedFirstPush),
			{
	 ...
	 */
	async getOperationSupport(filePath?: string): Promise<import("./client.js").LSPOperationSupport | null> {
		if (filePath) {
			const spawned = await this.getClientForFile(filePath);
			if (!spawned) return null;
	 ...
	 */
	async codeAction(filePath: string, line: number, character: number, endLine: number, endCharacter: number) {
		const spawned = await this.getClientForFile(filePath, NAV_CLIENT_WAIT_TIMEOUT_MS);
		if (!spawned) return [];
	 ...
	 */
	async getAllDiagnostics(): Promise<Map<string, { diags: import("./client.js").LSPDiagnostic[]; ts: number }>> {
		const all = new Map<string, { diags: import("./client.js").LSPDiagnostic[]; ts: number }>();
		const now = Date.now();
