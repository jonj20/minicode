...
function detectReact(_cwd: string, pkg: PackageJsonShape | undefined): FrameworkDetection | undefined {
...
function detectNext(cwd: string, pkg: PackageJsonShape | undefined): FrameworkDetection | undefined {
	const signals: string[] = [];
...
function detectVite(cwd: string, pkg: PackageJsonShape | undefined): FrameworkDetection | undefined {
	const signals: string[] = [];
...
function detectVitest(cwd: string, pkg: PackageJsonShape | undefined): FrameworkDetection | undefined {
	const signals: string[] = [];
	if (hasDep(pkg, "vitest")) signals.push("package.json:devDependencies.vitest");
	for (const candidate of ["vitest.config.ts", "vitest.config.js", "vitest.config.mjs", "vitest.config.mts"]) {
		if (fileExists(cwd, candidate)) signals.push(candidate);
	}
	if (signals.length === 0) return undefined;
...
export function detectProjectConventions(cwd: string): ProjectConventions {
	...
	const detectors: Array<(cwd: string, pkg: PackageJsonShape | undefined) => FrameworkDetection | undefined> = [
		detectReact,
		detectNext,
		detectVite,
		detectVitest,
