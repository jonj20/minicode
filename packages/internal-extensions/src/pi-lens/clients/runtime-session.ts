...
import { GovulncheckClient, type GovulncheckResult } from "./govulncheck-client.js";
...
import { findNearestProjectRoot, resolveStartupScanContext, type StartupScanContext } from "./startup-scan.js";
...
function scheduleStartupScans(
	...
	runTask("jscpd", async () => {
		if (await jscpdClient.ensureAvailable()) {
			...
			const cached = cacheManager.readCache<Awaited<ReturnType<JscpdClient["scan"]>>>(scannerKey, analysisRoot);
			if (cached) {
				if (!runtime.isCurrentSession(sessionGeneration)) return;
	...
	runTask("dead-code", async () => {
		...
		await Promise.all(
			applicable.map(async (client) => {
				...
				const cached = cacheManager.readCache<DeadCodeResult>(cacheKey, analysisRoot);
				if (cached) {
	...
	runTask("govulncheck", async () => {
		...
		if (!runtime.isCurrentSession(sessionGeneration)) return;
		const cached = cacheManager.readCache<GovulncheckResult>("govulncheck", analysisRoot);
		if (cached) {
			if (!runtime.isCurrentSession(sessionGeneration)) return;
	...
	runTask("gitleaks", async () => {
		...
		if (!runtime.isCurrentSession(sessionGeneration)) return;
		const cached = cacheManager.readCache<GitleaksResult>("gitleaks", analysisRoot);
		if (cached) {
			if (!runtime.isCurrentSession(sessionGeneration)) return;
	...
	runTask("madge", async () => {
		...
		if (!runtime.isCurrentSession(sessionGeneration)) return;
		const cached = cacheManager.readCache<{ circular: unknown[] }>("madge", analysisRoot);
		if (cached) {
	...
	runTask("call-graph", async () => {
		...
		const { buildOrUpdateGraph, extractSymbolsAndRefsFromGraph, isReviewGraphMigrationNeeded } = await import(
		...
		if (cached) {
			...
			if (stale.length === 0 && cachedFiles.length > 0 && !isReviewGraphMigrationNeeded(analysisRoot)) {
 ...
 */
export function getSessionStartGuidance(extendedToolsEnabled: boolean): string[] {
	const core =
		...
		"• lsp_navigation — definitions/references/hover/rename; probe LSP for code intelligence.\n" +
		"• ast_grep_search — structural code patterns (replaces regex for code intelligence).\n" +
		"• module_report + read_symbol/read_enclosing — navigable outline + exact body reads; cheaper than reading a whole file before editing.";
	if (!extendedToolsEnabled) return [core];
	return [
		core +
			"\n• lsp_diagnostics — proactive type/error checks before builds.\n" +
			"• lens_diagnostics — session-wide diagnostic state; mode=all resurfaces stale blocking errors.\n" +
			"• ast_grep_replace / ast_grep_outline / ast_grep_dump — extended ast-grep tools.",
	];
}
...
export async function handleSessionStart(deps: SessionStartDeps): Promise<void> {
	...
	const freshSnapshot = isProjectSnapshotFresh(snapshot, effectiveSeq) ? snapshot : null;
	if (freshSnapshot) {
		hydrateRuntimeFromProjectSnapshot(runtime, freshSnapshot);
	}

	const startupScanSource = freshSnapshot?.startupScan ? "snapshot" : "computed";
	...
	const agentStartupGuidance = getSessionStartGuidance(!!deps.getFlag("extended-tools"));
