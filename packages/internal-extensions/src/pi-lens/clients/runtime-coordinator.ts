...
export class RuntimeCoordinator {
	...
	private readonly _lspReadWarmState = new Map<string, { status: "warming" | "ready"; ts: number }>();
	private readonly _pendingInlineBlockers = new Map<string, { filePath: string; summary: string }>();
	...
	formatPipelineCrashNotice(filePath: string, err: unknown): string {
		...
		const shouldSurface =
			count <= RUNTIME_CONFIG.crashNotice.alwaysShowFirstN || count % RUNTIME_CONFIG.crashNotice.showEveryNth === 0;
	 ...
	 */
	async settleCascadeRuns(maxWaitMs: number): Promise<{ settled: number; timedOut: number }> {
		...
		const tracked = pending.map((p) => {
			const entry: { done: boolean; run?: CascadeRun; promise: Promise<CascadeRun> } = { done: false, promise: p };
	...
	deferFormat(filePath: string, cwd: string, toolName: "write" | "edit", turnStateCwd: string): void {
		const key = path.resolve(filePath);
		const now = Date.now();
