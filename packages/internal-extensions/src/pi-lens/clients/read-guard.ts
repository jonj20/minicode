...
const OWN_EDIT_STALE_GRACE_MS = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_READ_GUARD_OWN_EDIT_GRACE_MS ?? "120000", 10) || 120000,
);

/** Avoid hashing very large reads in the hot path. */
const READ_HASH_MAX_LINES = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_READ_GUARD_HASH_MAX_LINES ?? "3000", 10) || 3000,
);
...
const RELOCATION_WINDOW_MIN = Math.max(
	1,
	Number.parseInt(process.env.PI_LENS_READ_GUARD_RELOCATION_WINDOW_MIN ?? "40", 10) || 40,
);
const RELOCATION_WINDOW_PER_EDIT = Math.max(
	0,
	Number.parseInt(process.env.PI_LENS_READ_GUARD_RELOCATION_WINDOW_PER_EDIT ?? "20", 10) || 20,
);
const RELOCATION_WINDOW_MAX = Math.max(
	RELOCATION_WINDOW_MIN,
	Number.parseInt(process.env.PI_LENS_READ_GUARD_RELOCATION_WINDOW_MAX ?? "400", 10) || 400,
);
...
function readEffectiveRangeCoversRange(read: ReadRecord, [startLine, endLine]: [number, number]): boolean {
	return readRangeCoversLine(read, startLine) && readRangeCoversLine(read, endLine);
}

function captureLineHashes(filePath: string, offset: number, limit: number): Record<number, string> | undefined {
...
export class ReadGuard {
	...
	recordRead(record: ReadRecord): void {
		...
		const storedRecord: ReadRecord = {
			...record,
			filePath,
			lineHashes: record.lineHashes ?? captureLineHashes(filePath, record.effectiveOffset, record.effectiveLimit),
		};
	...
	checkEdit(
		...
		const effectiveMode: "block" | "warn" | undefined = exemptionMode === "warn" ? "warn" : undefined;
		...
		const rangesToCheck: [number, number][] = editRanges && editRanges.length > 1 ? editRanges : [touchedLines];
	...
	private canIgnoreStalenessByHashes(
		...
		return rangesToCheck.every((range) =>
			reads.some((read) => this.readCoversRange(read, range) && this.readRangeHashesStillMatch(read, lines, range)),
		);
	}
	...
	private checkCoverage(filePath: string, touchedLines: [number, number]): { covered: boolean; viaSymbol: boolean } {
