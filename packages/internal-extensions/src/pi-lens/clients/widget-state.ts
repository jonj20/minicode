...
export function recordFormatter(filePath: string, formatter: string, changed: boolean, success: boolean): void {
...
function formatFileRowVertical(rec: FileRecord, theme: { fg: (color: string, s: string) => string }): string {
	const dim = (s: string) => theme.fg("dim", s);
	const red = (s: string) => theme.fg("error", s);
	const yellow = (s: string) => theme.fg("warning", s);
	const green = (s: string) => theme.fg("success", s);
	...
	const counts =
		errors > 0
			? ` ${(blocking > 0 ? red : yellow)(`${errors}E`)}${warnings > 0 ? ` ${yellow(`${warnings}W`)}` : ""}`
			: warnings > 0
				? ` ${yellow(`${warnings}W`)}`
				: ` ${dim("clean")}`;
