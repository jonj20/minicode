...
import type { ProjectDiagnostic, ProjectDiagnosticsScanOptions, ProjectDiagnosticsSnapshot } from "./types.js";
...
async function scanFactRules(cwd: string, files: string[]): Promise<ProjectDiagnostic[]> {
	const facts = new FactStore();
	const pi = { getFlag: () => undefined };
	const diagnostics: ProjectDiagnostic[] = [];
	for (const filePath of files) {
		if (isTestFile(filePath) || !FACT_RULE_EXTENSIONS.has(path.extname(filePath))) {
			continue;
		}
...
async function scanTreeSitter(cwd: string, files: string[]): Promise<ProjectDiagnostic[]> {
 ...
 */
async function scanAstGrepNapi(cwd: string, files: string[]): Promise<ProjectDiagnostic[]> {
