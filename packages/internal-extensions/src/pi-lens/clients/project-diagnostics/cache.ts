...
export function loadProjectDiagnosticsSnapshot(cwd: string): ProjectDiagnosticsSnapshot | undefined {
	try {
		const parsed = JSON.parse(fs.readFileSync(cachePath(cwd, SNAPSHOT_CACHE_FILE), "utf-8")) as unknown;
...
export function saveProjectDiagnosticsSnapshot(cwd: string, snapshot: ProjectDiagnosticsSnapshot): void {
	const filePath = cachePath(cwd, SNAPSHOT_CACHE_FILE);
	fs.mkdirSync(path.dirname(filePath), { recursive: true });
	fs.writeFileSync(filePath, JSON.stringify(snapshot, null, 2));
}

export function loadProjectDiagnosticsDeltaReport(cwd: string): ProjectDiagnosticsDeltaReport | undefined {
	try {
		const parsed = JSON.parse(fs.readFileSync(cachePath(cwd, DELTA_CACHE_FILE), "utf-8")) as unknown;
		if (!parsed || typeof parsed !== "object") return undefined;
...
export function writeProjectDiagnosticsDeltaReport(cwd: string, report: ProjectDiagnosticsDeltaReport): void {
