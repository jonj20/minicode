...
function knipIssueFile(cwd: string, issue: KnipIssue): string {
	...
	return path.isAbsolute(issue.file) ? issue.file : path.resolve(cwd, issue.file);
}
...
export function knipIssueToProjectDiagnostic(cwd: string, issue: KnipIssue): ProjectDiagnostic {
...
export function knipIssuesToProjectDiagnostics(cwd: string, issues: KnipIssue[]): ProjectDiagnostic[] {
