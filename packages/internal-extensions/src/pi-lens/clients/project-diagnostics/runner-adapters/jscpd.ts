 ...
 */
export function jscpdCloneToProjectDiagnostics(cwd: string, clone: DuplicateClone): ProjectDiagnostic[] {
	const a = resolveClonePath(cwd, clone.fileA);
	const b = resolveClonePath(cwd, clone.fileB);

	const make = (filePath: string, line: number, otherFile: string, otherLine: number): ProjectDiagnostic => ({
		filePath,
		line,
		severity: "warning",
		semantic: "warning",
	...
	return [make(a, clone.startA, b, clone.startB), make(b, clone.startB, a, clone.startA)];
}
 ...
 */
export function jscpdResultToProjectDiagnostics(cwd: string, result: JscpdResult): ProjectDiagnostic[] {
