...
export function deadCodeIssueToProjectDiagnostic(
	...
	return {
		filePath: issue.file ? (path.isAbsolute(issue.file) ? issue.file : path.resolve(cwd, issue.file)) : cwd,
		line: issue.line,
...
export function deadCodeResultToProjectDiagnostics(cwd: string, result: DeadCodeResult): ProjectDiagnostic[] {
	if (!result.success) return [];
	const issues = [...result.unusedExports, ...result.unusedFiles, ...result.unusedDeps, ...result.unlistedDeps];
