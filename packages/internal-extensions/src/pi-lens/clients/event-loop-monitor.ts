...
const safeMs = (ns: number): number => (Number.isFinite(ns) ? Math.round((ns / NS_PER_MS) * 10) / 10 : 0);
 ...
 */
export function shouldLogWorstBlock(maxMs: number, lastLoggedMs: number, minMs = 60, deltaMs = 25): boolean {
