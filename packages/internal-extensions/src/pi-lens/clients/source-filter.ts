...
function shouldSkipGeneratedOrArtifact(
	filePath: string,
	options?: Pick<SourceCollectionOptions, "includeGenerated" | "includeDeclarationFiles" | "inspectGeneratedHeaders">,
): boolean {
...
function resolveCollectionConfig(rootDir: string, options?: SourceCollectionOptions): ResolvedCollectionConfig {
...
function classifyEntry(
	...
	if (entry.isDirectory()) {
		...
		if (options?.includeGenerated !== true && isGeneratedArtifactDirectoryName(entry.name)) {
			return {};
		}
