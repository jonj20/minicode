...
import { centralityFromReverseDeps, deserializeWordIndex, type RankedFile, searchWordIndex } from "./word-index.js";
...
export function projectScan(cwd: string, maxFiles?: number): Promise<ProjectDiagnosticsSnapshot> {
 ...
 */
export function symbolSearch(query: string, cwd: string, limit = 20): SymbolSearchResult {
	...
	const centrality = centralityFromReverseDeps(index, snapshot?.reverseDeps, (file) =>
