...
export class SgRunner {
	...
	private async probeCommand(cmd: string, argsPrefix: string[]): Promise<boolean> {
	...
	async tempScanAsync(dir: string, ruleId: string, ruleYaml: string, timeout = 30000): Promise<SgMatch[]> {
	...
	async tempScanWithFixAsync(
		...
		try {
			...
			const applyResult = await safeSpawnAsync(
				sgCmd,
				[...sgPre, "scan", "--config", configFile, "--update-all", ...sgExcludeArgsForProject(dir), dir],
				{ timeout },
			);
	 ...
	 */
	formatMatches(matches: SgMatch[], isDryRun = false, maxItems = 50, showModeIndicator = false): string {
		if (matches.length === 0) {
