...
export function getProjectDataDir(cwd: string): string {
	...
	const base = configuredBase || path.join(os.homedir(), ".minicode", ".pi-lens", "projects");
...
export function getGlobalPiLensDir(): string {
	return path.join(os.homedir(), ".minicode", ".pi-lens");
}
...
function matchesGitignorePattern(pattern: GitignorePattern, relativePath: string, isDirectory: boolean): boolean {
...
function buildProjectIgnoreMatcher(resolvedRoot: string, patterns: GitignorePattern[]): ProjectIgnoreMatcher {
	const nestedCache = new Map<string, { gitignoreMtimeMs: number; patterns: GitignorePattern[] }>();
...
function lensConfigInfo(rootDir: string): {
	...
	return info ? { info, path: info.path, mtimeMs: info.mtimeMs } : { info, path: undefined, mtimeMs: -1 };
}
...
export function isPathIgnoredByProject(filePath: string, rootDir: string, isDirectory = false): boolean {
