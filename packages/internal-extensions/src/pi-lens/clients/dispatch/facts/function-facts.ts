...
const FUNCTION_TYPES = new Set(["function_declaration", "method_definition", "function_expression", "arrow_function"]);
...
function namedChildren(node: TsNode): TsNode[] {
	return (node.children ?? []).filter((c: TsNode) => c?.isNamed && c.type !== "comment");
}
...
function getParameters(node: TsNode): TsNode[] {
	...
	return (fp.children ?? []).filter(
		(c: TsNode) =>
			c && (c.type === "required_parameter" || c.type === "optional_parameter" || c.type === "rest_pattern"),
	);
}
	...
	async run(ctx, store) {
		await extractFactsFromTree(ctx, store, { "file.functionSummaries": [] }, (root) => {
			...
			const addSummary = (node: TsNode): void => {
				...
				const passThrough =
					statementCount === 1 ? isCallPassThrough(statements[0], paramNames) : { pass: false as const };
