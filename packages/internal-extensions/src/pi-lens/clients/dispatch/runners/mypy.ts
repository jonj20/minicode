...
export function parseMypyOutput(raw: string, fallbackPath: string, cwd: string): Diagnostic[] {
	...
	for (const match of raw.matchAll(linePattern)) {
		...
		const filePath = file?.trim() ? (path.isAbsolute(file) ? file : path.resolve(cwd, file)) : fallbackPath;
		diagnostics.push({
...
const mypyRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const result = await safeSpawnAsync(cmd, ["--no-error-summary", "--show-column-numbers", ctx.filePath], {
			timeout: 30000,
			cwd,
		});
