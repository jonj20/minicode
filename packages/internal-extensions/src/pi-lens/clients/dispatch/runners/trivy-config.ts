...
import { isTrivyEnabled, resolveSeverityFloor, type TrivySeverity } from "../../trivy-client.js";
 ...
 */
export function parseTrivyConfigOutput(raw: string, filePath: string): Diagnostic[] {
	if (!raw.trim()) return [];
	let parsed: unknown;
	try {
	...
	for (const resultEntry of results) {
		...
		const rows = (resultEntry as { Misconfigurations?: unknown }).Misconfigurations;
...
const trivyConfigRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const result = await safeSpawnAsync(
			cmd,
			["config", "--quiet", "--no-progress", "--format", "json", "--severity", severities, absPath],
			{ cwd, timeout: 60_000 },
		);
