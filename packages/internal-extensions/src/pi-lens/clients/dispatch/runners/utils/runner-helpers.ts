 ...
 */
export function createVenvFinder(command: string, windowsExt = "", quoteWindows = false): (cwd: string) => string {
 ...
 */
export function createCwdCachedProbe(probe: (cwd: string) => Promise<boolean>): (cwd: string) => Promise<boolean> {
...
export function resolveVendorToolCommand(cwd: string, toolName: string, windowsExt = ".bat"): string | null {
	const isWin = process.platform === "win32";
...
async function probeAstGrepCommandAsync(cmd: string, argsPrefix: string[] = []): Promise<boolean> {
	...
	return !check.error && check.status === 0 && isAstGrepVersionOutput(`${check.stdout}\n${check.stderr}`);
}
...
function buildSgLocalBins(): string[] {
	...
	const binRoots = [...findNodeBinRoots(_thisDir), ...findNodeBinRoots(process.cwd()), _managedToolsDir];
	const bins: string[] = [];
