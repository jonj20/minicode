...
const cppCheckRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const diagnostics =
			compiler.flavor === "msvc" ? parseMsvcOutput(raw, ctx.filePath) : parseGccLikeOutput(raw, ctx.filePath);
					...
					diagnostics: [
						{
							...
							message:
								firstOutputLine(raw) || `${compiler.command} exited non-zero without structured diagnostics`,
