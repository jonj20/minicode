...
function parseBiomeJson(raw: string, filePath: string): { diagnostics: Diagnostic[]; parseError?: string } {
	try {
		...
		return {
			diagnostics: diagnostics.map((d) => ({
				...
				fixKind: d.tags?.includes("fixable") && autofix?.fixKind !== "none" ? autofix?.fixKind : undefined,
			})),
		};
	} catch (err) {
		return {
