...
function buildCodeActionSuggestion(actions: import("../../lsp/client.js").LSPCodeAction[]): string | undefined {
	if (!actions.length) return undefined;
	const quickFixes = actions.filter((action) => action.kind?.startsWith("quickfix"));
	if (!quickFixes.length) return undefined;

	const titles = Array.from(
		new Set(quickFixes.map((action) => normalizeActionTitle(action.title)).filter((title) => title.length > 0)),
...
const lspRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const diagnostics: Diagnostic[] = convertLspDiagnostics(validLspDiags, diagnosticPath, { fixSuggestionByIndex });
