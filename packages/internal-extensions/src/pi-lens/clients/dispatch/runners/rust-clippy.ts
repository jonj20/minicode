...
function findMachineApplicableSpan(spans: ClippySpan[]): ClippySpan | undefined {
	return spans.find(
		(s) => typeof s.suggested_replacement === "string" && s.suggestion_applicability === "MachineApplicable",
	);
}

export function parseClippyOutput(raw: string, fallbackPath: string, cargoDir?: string): Diagnostic[] {
