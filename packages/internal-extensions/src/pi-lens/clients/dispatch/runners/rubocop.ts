		...
		for (const file of output.files) {
			for (const offense of file.offenses) {
				...
				diagnostics.push({
					...
					fixKind: offense.correctable && autofix?.fixKind !== "none" ? autofix?.fixKind : undefined,
				});
			}
		}
...
const rubocopRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const result = await safeSpawnAsync(cmd, [...args, "--format", "json", "--no-color", ctx.filePath], {
			timeout: 30000,
			cwd,
		});
