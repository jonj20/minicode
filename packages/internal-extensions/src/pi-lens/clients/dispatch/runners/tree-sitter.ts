...
import { isDisabledQueryFilePath, queryLoader, type TreeSitterQuery } from "../../tree-sitter-query-loader.js";
import {
 ...
 */
function getTotalLinesChanged(ranges: ReadonlyArray<{ start: number; end: number }> | undefined): number {
				...
				batch.map(async (query) => {
					...
					try {
						...
						for (const match of matches) {
							...
							const semantic =
								query.severity === "error" ? "blocking" : query.severity === "warning" ? "warning" : "none";
							const defectClass =
