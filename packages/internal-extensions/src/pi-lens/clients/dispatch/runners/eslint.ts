...
const eslintProbeByCmd = new Map<string, ReturnType<typeof makeEslintProbe>>();
...
function parseEslintJson(raw: string, filePath: string): { diagnostics: Diagnostic[]; parseError?: string } {
	try {
...
const eslintRunner: RunnerDefinition = {
	...
	async run(ctx: DispatchContext): Promise<RunnerResult> {
		...
		const result = await safeSpawnAsync(cmd, ["--format", "json", "--no-error-on-unmatched-pattern", ctx.filePath], {
			timeout: 30000,
			cwd,
		});
