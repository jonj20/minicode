...
export function enabledAuxiliaryLspServerIds(getFlag: GetFlag): string[] {
	return AUXILIARY_LSP_PROFILES.flatMap((p) =>
		p.enabledByDefault && !(p.killSwitchFlag && getFlag(p.killSwitchFlag) === true) ? [p.serverId] : [],
	);
}

/** Find the profile whose server emitted a diagnostic with this `source`. */
export function findAuxiliaryProfileForSource(source: string | undefined): AuxiliaryLspProfile | undefined {
