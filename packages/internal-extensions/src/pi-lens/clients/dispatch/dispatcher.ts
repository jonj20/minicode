...
import { getRunnerTimeoutFloorMs, RUNTIME_CONFIG } from "../runtime-config.js";
...
async function checkToolAvailability(command: string, facts: FactStore): Promise<boolean> {
...
function promoteDeltaUnusedToBlockers(diagnostics: Diagnostic[]): Diagnostic[] {
	return diagnostics.map((d) => {
		...
		return {
			...
			semantic: "blocking",
			fixSuggestion:
				d.fixSuggestion ?? "Remove the unused declaration or rename with '_' prefix if intentionally unused.",
		};
	});
}
...
function buildCoverageNotice(ctx: DispatchContext, runnerLatencies: RunnerLatency[]): Diagnostic | undefined {
	...
	const STRUCTURAL_RUNNERS = new Set(["tree-sitter", "ast-grep-napi", "spellcheck", "fact-rules", "opengrep"]);
