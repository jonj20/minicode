...
const WEIGHTS: Record<string, number> = {
	code: 0.3, // 30% - console.log, TODO, empty catch, as any, debugger
	tests: 0.2, // 20% - test files, test cases, framework config
	docs: 0.2, // 20% - README, LICENSE, CHANGELOG, pkg metadata
...
function validateDocs(root: string): CategoryResult {
	...
	for (const { file, name } of required) {
		const exists =
			fs.existsSync(path.join(root, file)) || (file === "LICENSE" && fs.existsSync(path.join(root, "LICENSE.md")));
		if (exists) {
			found.push(name);
		} else {
			missing.push(name);
...
function validateDeploy(root: string): CategoryResult {
	...
	if (fs.existsSync(packageJsonPath)) {
		try {
			...
			const hasBuildScript = pkg.scripts && (pkg.scripts.build || pkg.scripts.compile || pkg.scripts["build:prod"]);
			if (hasBuildScript) {
