...
export const TOOLS: ToolDefinition[] = [
	...
	},
	{
		...
		github: {
			...
			assetMatch: (platform, arch) => {
				if (platform === "linux") return arch === "arm64" ? "opengrep_manylinux_aarch64" : "opengrep_manylinux_x86";
				if (platform === "darwin") return arch === "arm64" ? "opengrep_osx_arm64" : "opengrep_osx_x86";
	...
	},
	{
		...
		archive: {
			url: (platform, arch) => {
				...
				if (platform === "linux") return arch === "x64" ? `${base}/clangd-linux-${version}.zip` : undefined;
	...
	},
	{
		...
		github: {
			...
			assetMatch: (platform, arch) => {
				if (platform === "linux") return arch === "arm64" ? "linux_arm64.tar.gz" : "linux_amd64.tar.gz";
				if (platform === "darwin") return arch === "arm64" ? "darwin_arm64.tar.gz" : "darwin_amd64.tar.gz";
	...
	},
	{
		...
		github: {
			...
			assetMatch: (platform, arch) => {
				if (platform === "linux") return arch === "arm64" ? "native-linux-aarch64.zip" : "native-linux-amd64.zip";
				if (platform === "darwin") return arch === "arm64" ? "native-macos-aarch64.zip" : "native-macos-amd64.zip";
	...
	},
	{
		...
		github: {
			...
			assetMatch: (platform, arch) => {
				if (platform === "linux") return arch === "arm64" ? "marksman-linux-arm64" : "marksman-linux-x64";
...
const PROBE_CACHE_PATH = path.join(os.homedir(), ".minicode", ".pi-lens", "probe-cache.json");
...
function scheduleProbeFlush(): void {
	...
	_probeCacheFlushTimer = setTimeout(() => {
		...
		void fs.writeFile(PROBE_CACHE_PATH, JSON.stringify(_probeCache, null, 2)).catch(() => {});
	}, 300);
...
// Exported for testing only.
export async function updateProbeCache(toolId: string, resolvedPath: string): Promise<void> {
	try {
 ...
 */
async function isCommandAvailable(command: string, _args?: string[]): Promise<boolean> {
...
async function getArchiveTreeBundlePath(tool: ToolDefinition): Promise<string | undefined> {
	...
	const marker = tool.archive?.treeMarker ? path.join(extractDir, ...tool.archive.treeMarker.split("/")) : extractDir;
	try {
...
function getGitHubInstalledBinaryName(binaryName: string, platform: string, assetName: string): string {
...
function getArchiveBinaryCandidates(binaryName: string, platform: string, assetName: string): string[] {
	if (platform !== "win32") return [binaryName];
	if (hasExecutableExtension(binaryName)) return [binaryName];
...
async function findNpmGlobalToolPath(binaryName: string): Promise<string | undefined> {
	const isWindows = process.platform === "win32";
	const binDirs = await getNpmGlobalBinCandidates();

	for (const dir of binDirs) {
		const candidates = isWindows
			? [path.join(dir, `${binaryName}.cmd`), path.join(dir, `${binaryName}.exe`), path.join(dir, binaryName)]
...
async function findPipUserToolPath(binaryName: string): Promise<string | undefined> {
	const isWindows = process.platform === "win32";
	...
	for (const userBase of userBaseCandidates) {
		...
		for (const dir of scriptDirs) {
			const candidates = isWindows
				? [path.join(dir, `${binaryName}.exe`), path.join(dir, `${binaryName}.cmd`), path.join(dir, binaryName)]
				: [path.join(dir, binaryName)];
 ...
 */
function httpsGet(url: string, maxRedirects = 5, headers: Record<string, string> = {}): Promise<Buffer> {
...
async function installArchiveTool(tool: ToolDefinition): Promise<string | undefined> {
	...
	try {
		...
		const tarBin = isWindows ? `${process.env.SystemRoot ?? "C:\\Windows"}\\System32\\tar.exe` : "tar";
		const extracted = await new Promise<{ ok: boolean; stderr: string }>((resolve) => {
		...
		if (!spec.launcher) {
			const marker = spec.treeMarker ? path.join(extractDir, ...spec.treeMarker.split("/")) : extractDir;
			try {
				await fs.access(marker);
			} catch {
...
async function installNpmTool(packageName: string, binaryName: string): Promise<string | undefined> {
	try {
		...
		const erResolve =
			outcome.ok === false && /npm\s+error\s+ERESOLVE|\bERESOLVE\b|could not resolve/i.test(outcome.stderr);
	...
	try {
		...
		for (const candidate of pipCandidates) {
			...
			if (outcome.ok) {
				...
				if (userBaseResult) {
					const candidateScriptDirs: string[] = [path.join(userBaseResult, isWindows ? "Scripts" : "bin")];
...
export async function installTool(toolId: string): Promise<boolean> {
	...
	try {
		...
		logSessionStart(`auto-install ${tool.id}: exception ${(err as Error).message} (${Date.now() - startedAt}ms)`);
		return false;
	}
}
 ...
 */
export async function checkAllTools(): Promise<Array<{ id: string; name: string; installed: boolean; path?: string }>> {
	const results = [];
	for (const tool of TOOLS) {
 ...
 * Exported for testing only.
 */
export function resolveGitHubAsset(toolId: GitHubToolId, platform: string, arch: string): string | undefined {
	const tool = TOOLS.find((t) => t.id === toolId);
