...
import { detectNodePackageManager, formatRunScript, type NodePackageManager } from "./package-manager.js";
...
function detectNodeProject(targetPath: string): ProjectMetadata | null {
	...
	if (
		allDeps.eslint ||
		fs.existsSync(path.join(targetPath, ".eslintrc")) ||
	...
	if (
		allDeps.prettier ||
		fs.existsSync(path.join(targetPath, ".prettierrc")) ||
...
function detectRustProject(targetPath: string): ProjectMetadata | null {
	...
	if (fs.existsSync(path.join(targetPath, ".clippy.toml")) || fs.existsSync(path.join(targetPath, "clippy.toml"))) {
		metadata.hasLinting = true;
		metadata.linter = "clippy";
	}

	// Check for rustfmt
	if (fs.existsSync(path.join(targetPath, ".rustfmt.toml")) || fs.existsSync(path.join(targetPath, "rustfmt.toml"))) {
		metadata.hasFormatting = true;
...
function detectGoProject(targetPath: string): ProjectMetadata | null {
	...
	if (
		fs.existsSync(path.join(targetPath, ".golangci.yml")) ||
...
function detectPhpProject(targetPath: string): ProjectMetadata | null {
	...
	const metadata: ProjectMetadata = {
		...
		scripts:
			typeof composerJson.scripts === "object" && composerJson.scripts !== null
...
function detectJavaProject(targetPath: string): ProjectMetadata | null {
	...
	const hasGradle =
		fs.existsSync(path.join(targetPath, "build.gradle")) || fs.existsSync(path.join(targetPath, "build.gradle.kts"));
...
function detectCppProject(targetPath: string): ProjectMetadata | null {
	...
	const hasCppFiles =
		fs.existsSync(targetPath) && fs.readdirSync(targetPath).some((f) => /\.(cpp|cxx|cc|c\+\+)$/i.test(f));
