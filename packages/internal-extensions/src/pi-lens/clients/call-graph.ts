...
import type { SymbolRef, Symbol as SymbolType } from "./symbol-types.js";
...
export interface ResolvedCallEdge {
	...
	callerSymbol?: string;
	callerKey: SymbolTypeKey;
	calleeFile: string;
	calleeSymbol: string;
	calleeKey: SymbolTypeKey;
	/**
...
export function impact(
	...
	startKey: SymbolTypeKey,
	maxDepth = 3,
	minWeight = 0.1,
): ImpactResult[] {
	const results: ImpactResult[] = [];
	const visited = new Set<SymbolKey>([startKey]);
	const queue: Array<{ key: SymbolTypeKey; depth: number }> = [{ key: startKey, depth: 0 }];
 ...
 */
function buildDefIndex(allSymbols: Map<string, symbol[]>): Map<string, SymbolKey[]> {
	const index = new Map<string, SymbolKey[]>();
	for (const [, symbols] of allSymbols) {
		for (const sym of symbols) {
			if (!sym.name) continue;
			const key: SymbolTypeKey = `${sym.filePath}:${sym.name}`;
 ...
 */
function findEnclosingSymbol(symbols: SymbolType[], refLine: number): SymbolType | undefined {
	let best: SymbolType | undefined;
	for (const sym of symbols) {
...
export function buildCallGraph(
	...
	for (const [callerFile, refs] of allRefs) {
		...
		for (const ref of refs) {
			...
			const callerKey: SymbolTypeKey = enclosing ? `${callerFile}:${enclosing.name}` : `file:${callerFile}`;
 ...
 */
export function saveCallGraph(cwd: string, graph: FunctionCallGraph, fileMtimes: Map<string, number>): void {
 ...
 */
export function staleFiles(fileMtimes: Map<string, number>, currentFiles: string[]): string[] {
