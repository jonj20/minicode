 ...
 * `npm`.
 */
export async function resolveNodePackageManager(cwd: string = process.cwd()): Promise<NodePackageManager> {
 ...
 */
export function installArgs(pm: NodePackageManager, pkg: string, options: InstallOptions = {}): string[] {
...
async function globalBinDirsFor(pm: NodePackageManager): Promise<string[]> {
	...
	const query = pm === "npm" ? ["config", "get", "prefix"] : pm === "pnpm" ? ["bin", "-g"] : ["global", "bin"]; // yarn
 ...
 */
export async function findGlobalBinary(command: string, windowsExt = ".cmd"): Promise<string | undefined> {
...
function findLocalBinUpwards(tool: string, startDir: string, windowsExt: string): string | undefined {
 ...
 */
export async function findNodeToolBinary(tool: string, cwd: string, windowsExt = ".cmd"): Promise<string | undefined> {
