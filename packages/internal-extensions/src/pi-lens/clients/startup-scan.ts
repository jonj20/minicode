...
export const PROJECT_ROOT_MARKERS = [".git", "package.json", "pyproject.toml", "Cargo.toml", "go.mod", "composer.json"];
...
export function resolveStartupScanContext(cwd: string, options: StartupScanOptions = {}): StartupScanContext {
	const cacheKey = `${path.resolve(cwd)}|${options.homeDir ?? ""}|${options.maxSourceFiles ?? ""}`;
...
function computeStartupScanContext(cwd: string, options: StartupScanOptions = {}): StartupScanContext {
