...
function joinMessages(consumed: { messages: Array<{ content: string }> } | undefined): string | undefined {
 ...
 */
export async function runTurnEnd(cwd: string, files: string[] = []): Promise<TurnEndOutcome> {
	const ctx = await getMcpSessionContext();
	const host = createMcpHost();
