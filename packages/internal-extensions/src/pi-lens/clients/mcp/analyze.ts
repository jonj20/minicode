 ...
 */
async function warmLspForFile(absPath: string, host: ReturnType<typeof createMcpHost>): Promise<void> {
...
export async function analyzeFile(
	...
	const latencyReport =
		newReports.find((report) => path.resolve(report.filePath) === absPath) ?? newReports[newReports.length - 1];
