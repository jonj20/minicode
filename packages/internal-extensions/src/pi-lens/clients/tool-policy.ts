...
export function listSafePipelineAutofixTools(): string[] {
	return [...AUTOFIX_CAPABILITIES.entries()].filter(([, cap]) => cap.safePipelineAutofix).map(([id]) => id);
}
...
const YAMLLINT_CONFIGS = [".yamllint", ".yamllint.yml", ".yamllint.yaml", "pyproject.toml", "setup.cfg", "tox.ini"];
...
export function getLinterPolicyForFile(filePath: string, context: LinterPolicyContext = {}): LinterPolicy | undefined {
...
export function getPreferredAutofixTools(filePath: string, context: AutofixPolicyContext): AutofixToolName[] {
...
const KTFMT_GRADLE_FILES = ["build.gradle.kts", "build.gradle", "settings.gradle.kts", "settings.gradle"];
...
export function hasOxlintConfig(cwd: string): boolean {
	for (const dir of walkUpDirsUntilPackageJson(cwd)) {
		if (fs.existsSync(path.join(dir, ".oxlintrc.json")) || fs.existsSync(path.join(dir, "oxlint.json"))) return true;
	}
