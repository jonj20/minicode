...
export function consumeTurnEndFindings(
	...
	cacheManager.writeCache("turn-end-findings", null as unknown as { content: string }, cwd);
...
export function consumeTestFindings(
	...
	const findings = cacheManager.readCache<{ content: string }>("test-runner-findings", cwd);
	if (!findings?.data?.content) return;

	cacheManager.writeCache("test-runner-findings", null as unknown as { content: string }, cwd);
