...
import { type ActionableWarningRecord, recordFromDispatchDiagnostic } from "./actionable-warnings.js";
import type { BiomeClient } from "./biome-client.js";
import { type CodeQualityWarningRecord, recordFromCodeQualityDiagnostic } from "./code-quality-warnings.js";
...
async function tryRustClippyFix(filePath: string): Promise<string[]> {
	...
	const result = await safeSpawnAsync("cargo", ["clippy", "--fix", "--allow-dirty", "--allow-staged", "-q"], {
		timeout: 30000,
		cwd: cargoDir,
	});
...
function buildAllClearOutput(_dispatchResult: DispatchResult, elapsed: number, filePath: string): string {
...
export async function runFormatPhase(
	...
	try {
		...
		if (result.anyChanged) {
			...
			dbg(
				"autoformat: " +
					result.formatters.map((f) => `${f.name}(${f.changed ? "changed" : "unchanged"})`).join(", "),
			);
		}
		if (!result.allSucceeded) {
			const failures = result.formatters.filter((f) => !f.success);
			formatFailures.push(...failures.map((f) => `${f.name}: ${f.error ?? "unknown error"}`));
			dbg(`autoformat: ${failures.map((f) => `${f.name} failed: ${f.error ?? "unknown error"}`).join("; ")}`);
		}
	} catch (err) {
...
export async function runPipeline(ctx: PipelineContext, deps: PipelineDeps): Promise<PipelineResult> {
	...
	if (dispatchResult.resolvedCount > 0) getDiagnosticTracker().trackAgentFixed(dispatchResult.resolvedCount);
	...
	return {
		...
		fileModified,
		changedFiles,
		inlineBlockerSummary: dispatchResult.hasBlockers ? dispatchResult.blockerOutput.trim() || undefined : undefined,
		actionableWarnings,
		codeQualityWarnings,
	};
}
