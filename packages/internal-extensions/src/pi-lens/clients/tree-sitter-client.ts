...
export class TreeSitterClient {
	...
	private dbg(msg: string): void {
		if (this.verbose) {
			console.error(`[tree-sitter] ${msg}`); // pi-lens-ignore: console-statement â€?intentional verbose logger
		}
	}

	/**
	 * Resolve a web-tree-sitter asset path using multiple strategies:
	 * 1. Node module resolution via createRequire (handles hoisted installs â€?issue #20)
	...
	private resolveWebTreeSitterAsset(asset: string): string | undefined {
		...
		// This is required when pi compiles TS on-the-fly to a temp directory â€?		// createRequire(import.meta.url) resolves from the temp dir and can't find
	...
	private findGrammarsDir(): string {
		...
		// (it is not a pi-lens dependency â€?grammars ship bundled / lazy-fetched).
		try {
	 ...
	 * whether or not it exists yet â€?so we can create + populate it when the
	 ...
	 * postinstall didn't (pnpm/bun skip lifecycle scripts â€?the documented
	...
	private async ensureGrammar(grammarFile: string): Promise<boolean> {
		...
		const task = (async (): Promise<boolean> => {
			const dir = this.grammarsDir && fs.existsSync(this.grammarsDir) ? this.grammarsDir : this.grammarsWriteDir();
			if (!dir) return false;
			// Reuse the shared single-file downloader (same CDN/source as the
			// postinstall) â€?see clients/grammar-source.ts.
			...
			if (ok) {
				...
				// instead of failing silently â€?otherwise pnpm/bun users offline get
				// no signal that a language's tree-sitter features are unavailable.
				console.error(
					`[pi-lens] tree-sitter grammar '${grammarFile}' is unavailable â€?` +
	...
	private async loadLanguage(languageId: string): Promise<TreeSitterLanguage | null> {
		...
		// never be loaded â€?skip it and degrade to "unavailable" (#423/#432). The
		// grammar-health nightly is what decides membership of BLOCKED_GRAMMARS.
		const blockReason = grammarBlockReason(grammarFile);
		if (blockReason) {
			this.dbg(`Grammar ${grammarFile} blocked on this runtime â€?${blockReason}`);
			return null;
		}
	...
	async parseFile(filePath: string, languageId: string, contentOverride?: string): Promise<TreeSitterTree | null> {
	...
	extractInjections(
		...
		let sqlMatch = sqlPattern.exec(content);
		while (sqlMatch !== null) {
			const lines = content.slice(0, sqlMatch.index).split("\n");
			injections.push({
				type: "sql",
				content: sqlMatch[2],
				line: lines.length,
			...
			sqlMatch = sqlPattern.exec(content);
		}

		// Pattern: styled.div`color: red;` or css`...`
		const cssPattern = /\b(styled(?:\.\w+)?|css)\s*`([^`]+)`/gi;
		let cssMatch = cssPattern.exec(content);
		while (cssMatch !== null) {
			const lines = content.slice(0, cssMatch.index).split("\n");
			injections.push({
				type: "css",
				content: cssMatch[2],
				line: lines.length,
			...
			cssMatch = cssPattern.exec(content);
		}

		// Pattern: new RegExp(`pattern`)
		const regexPattern = /new\s+RegExp\s*\(\s*`([^`]+)`/gi;
		let regexMatch = regexPattern.exec(content);
		while (regexMatch !== null) {
			const lines = content.slice(0, regexMatch.index).split("\n");
			injections.push({
				type: "regex",
			...
			regexMatch = regexPattern.exec(content);
		}
	...
	isAvailable(): boolean {
		// Available if the core TS grammar resolves in ANY source dir â€?the bundled
	...
	private getQueryCacheKey(pattern: string, languageId: string): string {
		...
		for (let i = 0; i < pattern.length; i++) {
			...
			hash = ((hash << 5) - hash + char) | 0; // NOSONAR: intentional 32-bit truncation for hash stability, not floatâ†’int conversion
		}
	...
	private isLikelySqlAlchemyReceiver(text: string): boolean {
		...
		return new Set(["session", "db_session", "async_session", "sync_session"]).has(tail.toLowerCase());
	}
	 ...
	 * Each branch is an independent filter identified by name â€?flat dispatch, no nesting.
	 */
	private applyPostFilter(
		...
		function extractSlots(classNode: any): string[] | null {
			...
			for (const stmt of body.children ?? []) {
				...
				if (!rhs) continue;
				if (rhs.type === "string") {
					// __slots__ = "a" â€?single string form (Python quirk)
		...
		switch (postFilter) {
			...
			case "count_params": {
				...
				// Count only truly required params â€?exclude:
				//   â€?optional_parameter nodes (foo?: T) if the grammar uses that type
				//   â€?required_parameter nodes that have a "?" child (same semantic,
				//     different grammar version â€?web-tree-sitter-typescript collapses
				//     both into required_parameter with a "?" token child)
				//   â€?params with a default value (text contains "=")
			...
			case "empty_body": {
				...
				const meaningful = bodyNode.children.filter(
					(c: any) => c.isNamed && c.type !== "comment" && c.type !== "line_comment" && c.type !== "block_comment",
				);
			...
			case "bare_except_only": {
				...
				// spec â€?one of: identifier (e.g. `except ValueError`),
			...
			case "open_mode_invalid": {
				...
				// Skip single-char modes (r/w/a/x â€?always valid)
			...
			case "format_arity_mismatch": {
				...
				// Don't strip a leading "%" â€?the format string's contents are
				...
				if (namedKeys.length > 0) {
					...
					if (argsNode.type === "dictionary") {
						...
						for (const child of argsNode.children ?? []) {
							...
							if (child.type === "pair" && child.children?.[0]) {
								...
								// Strip quotes â€?child is a string literal node,
					...
					// Format uses named but RHS isn't a dict â€?definitely wrong
					return true;
				}
			...
			case "slots_attribute_mismatch": {
				...
				// (a real S8494 violation â€?the assignment will raise AttributeError).
			...
			case "check_secret_pattern": {
				...
				// Skip UPPER_CASE constants â€?they're module-level constants
			...
			case "returns_error": {
				...
				const funcNode = this.navigator.findParent(first, ["function_declaration", "method_declaration"]);
				...
				const signature =
					String(funcNode.text ?? "")
						...
						?.trim() ?? "";
				const returnPart =
					signature.match(/func\s*(?:\([^)]*\)\s*)?[A-Za-z_]\w*\s*\([^)]*\)\s*(.*)$/s)?.[1]?.trim() ?? "";
			...
			case "ts_ssrf_sink": {
				...
				// many false positives â€?those need data-flow analysis to resolve.
				...
				return new Set(["axios", "http", "https", "got", "request", "superagent", "undici"]).has(obj);
			}
			...
			case "incomplete_assertion": {
				...
				if (current.type === "expression_statement" || current.type === "return_statement") return true;
			...
			case "py_command_injection_sink": {
				...
				return (
					...
					(mod === "subprocess" && /^(run|Popen|call|check_output|check_call)$/.test(fn) && kw === "shell")
				);
			}
			...
			case "go_sql_injection_sink":
				return (
					/^(Query|QueryContext|QueryRow|QueryRowContext|Exec|ExecContext)$/.test(captures.DBFN?.text ?? "") &&
	...
	private evaluatePredicates(query: any, match: any): boolean {
		const predicates: Array<(captures: unknown) => boolean> = query.textPredicates?.[match.patternIndex] ?? [];
	...
	private async searchFileWithQuery(
		...
		try {
			...
			for (const match of queryMatches) {
				...
				if (postFilter && !this.applyPostFilter(postFilter, postFilterParams, captures)) {
					continue;
				}
	...
	private collectFiles(dir: string, languageId: string, fileFilter?: (path: string) => boolean): string[] {
		const files: string[] = [];
		...
		// has already enumerated the whole tree â€?so a misrooted structuralSearch
