 ...
 */
export function parseVultureOutput(output: string, root: string): DeadCodeIssue[] {
...
export class PythonDeadCodeClient implements DeadCodeClient {
	...
	constructor(verbose = false) {
		this.log = verbose ? (msg: string) => console.error(`[dead-code:python] ${msg}`) : () => {};
	}
	 ...
	 */
	private resolveProjectRoot(startDir: string, homeDirOverride?: string): string | null {
		const markers = ["pyproject.toml", "setup.py", "setup.cfg", "requirements.txt", "Pipfile", "tox.ini"];
...
export function deadCodeIssueCount(result: DeadCodeResult): number {
	return (
		result.unusedExports.length + result.unusedFiles.length + result.unusedDeps.length + result.unlistedDeps.length
	);
}
 ...
 */
export function formatDeadCodeAdvisory(results: DeadCodeResult[], maxPerLang = 10): string {
