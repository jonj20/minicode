...
export async function safeSpawnAsync(
	...
	const abortSignal = options?.signal ?? (options?.ignoreAmbientSignal ? undefined : ambientAbortSignal);
	...
	return new Promise((resolve) => {
		...
		const spawnCmd = isWindows ? buildWindowsShellCommand(command, args) : command;
 ...
 */
export function safeSpawn(command: string, args: string[], options?: SafeSpawnOptions): SpawnResult {
