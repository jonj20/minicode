 ...
 */
function extractDiagnosticLines(output: string, predicate: (line: string) => boolean, maxLines: number = 10): string {
	if (!output) return "";
 ...
 */
export function sanitizeToolOutput(output: string, maxSummaryLength: number = 140): SanitizedResult {
	if (!output || typeof output !== "string") {
