...
export class DependencyChecker {
	...
	private scanInFlight = new Map<string, Promise<{ circular: CircularDep[]; count: number }>>();
	...
	constructor(verbose = false) {
		this.log = verbose ? (msg: string) => console.error(`[deps] ${msg}`) : () => {};
	}
	 ...
	 * fallback).
	 */
	private async resolveMadge(cwd: string): Promise<{ cmd: string; prefix: string[] }> {
	...
	extractImports(filePath: string): Set<string> {
		...
		let requireMatch = requirePattern.exec(content);
		while (requireMatch !== null) {
			if (requireMatch[1].startsWith(".")) {
				imports.add(requireMatch[1]);
			}
			requireMatch = requirePattern.exec(content);
		}
	...
	private async runCheckFile(normalized: string, projectRoot: string): Promise<DepCheckResult> {
	 ...
	 */
	async scanProject(cwd?: string): Promise<{ circular: CircularDep[]; count: number }> {
		const projectRoot = path.resolve(cwd || process.cwd());
	...
	private async runScanProject(projectRoot: string): Promise<{ circular: CircularDep[]; count: number }> {
		try {
			const { cmd, prefix } = await this.resolveMadge(projectRoot);
