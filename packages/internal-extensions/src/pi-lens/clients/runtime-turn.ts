...
function emitIdleResetReporterWarning(reportErr: unknown): void {
	try {
		process.emitWarning(`pi-lens LSP idle reset error reporter failed: ${reportErr}`, {
...
export async function handleTurnEnd(deps: TurnEndDeps): Promise<void> {
	...
	if (runtime.isStartupScanInFlight("knip")) {
		...
		if (previousFailedHard) {
			...
			if (knipResult.success && knipResult.issues.length > 0) {
				const issueKey = (i: KnipIssue) => `${i.type}:${i.file ?? ""}:${i.name}:${i.line ?? 0}:${i.package ?? ""}`;
	...
	const govCacheEntry = cacheManager.readCache<GovulncheckResult>("govulncheck", cwd);
	...
	const gitleaksData = cacheManager.readCache<GitleaksResult>("gitleaks", cwd)?.data;
	const astSecretWarnings = runtime.peekActionableWarnings().filter(isSecretWarning);
	...
	try {
		...
		for (const client of deadCodeClients) {
			...
			const cached = cacheManager.readCache<DeadCodeResult>(`dead-code-${client.id}`, cwd);
	...
	if (findingParts.length > 0) {
		...
		if (last?.data?.signature === signature && last?.data?.sessionId === runtime.telemetrySessionId) {
