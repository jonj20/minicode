...
import { type ExtractedSymbols, TreeSitterSymbolExtractor } from "../tree-sitter-symbol-extractor.js";
...
type SeqFastpathFallback = "no-seq" | "too-many-changes" | "new-file" | "verify-due" | "removed-file" | "stat-error";
 ...
 */
async function sourceHashMapAsync(files: string[]): Promise<Map<string, string>> {
 ...
 */
function diffSignatureMaps(previous: Map<string, string>, next: Map<string, string>): SignatureDelta {
	const added: string[] = [];
...
function getReviewGraphMaxFiles(): number {
	...
	return Number.isFinite(override) && override > 0 ? override : RUNTIME_CONFIG.reviewGraph.maxFiles;
}
...
function writePending(key: string): void {
	...
	try {
		...
		console.error("[review-graph] cache serialize failed:", (err as Error).message);
		return;
	}
	logLatency({
		type: "phase",
	...
	process.once("exit", () => {
		for (const [, pending] of _pendingPersist) {
			try {
				...
				fs.writeFileSync(pending.cachePath, JSON.stringify(pending.data), "utf-8");
			} catch {
...
async function ensureReviewGraphFacts(filePath: string, cwd: string, facts: FactStore): Promise<void> {
	...
	try {
		...
		console.error(
			`[pi-lens] review-graph structural facts disabled (degraded mode): ${(err as Error)?.message ?? String(err)}`,
		);
	}
}
...
function addJsTsFile(graph: ReviewGraph, cwd: string, filePath: string, facts: FactStore): void {
	...
	const functions = facts.getFileFact<FunctionSummary[]>(normalized, "file.functionSummaries") ?? [];
...
function mapKindToTreeSitterLanguage(kind: string | undefined, filePath?: string): string | undefined {
	switch (kind) {
		case "python":
			return "python";
		case "go":
...
async function extractTreeSitterSymbols(filePath: string, languageId: string): Promise<ExtractedSymbols> {
...
function ensureFileNode(graph: ReviewGraph, filePath: string, languageId: string): string {
	const normalized = normalizeMapKey(filePath);
...
function addCxxIncludeEdges(graph: ReviewGraph, cwd: string, filePath: string): void {
	let content = "";
	try {
...
async function addFileToGraph(graph: ReviewGraph, cwd: string, file: string, facts: FactStore): Promise<void> {
	const kind = detectFileKind(file);
...
function restoreValidIncomingEdges(graph: ReviewGraph, edges: ReviewGraphEdge[]): void {
	const existing = new Set(
		graph.edges.map(
			(edge) => `${edge.from}\u0000${edge.to}\u0000${edge.kind}\u0000${JSON.stringify(edge.metadata ?? {})}`,
		),
	);
...
async function updateGraphFiles(graph: ReviewGraph, cwd: string, files: string[], facts: FactStore): Promise<void> {
...
async function trySeqFastpath(
	...
	const now = Date.now();
	const ageMs = cached.lastFullVerifyMs === undefined ? Number.POSITIVE_INFINITY : now - cached.lastFullVerifyMs;
...
async function _doBuildGraph(
	...
	if (seqHint && seqAtBuildStart !== undefined && seqFastpathEnabled()) {
		const fast = await trySeqFastpath(cwd, normalizedCwd, normalizedChanged, facts, seqHint, seqAtBuildStart);
