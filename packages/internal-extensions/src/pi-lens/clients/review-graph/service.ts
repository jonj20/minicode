...
export function computeImpactCascade(graph: ReviewGraph, changedFile: string, cwd?: string): ImpactCascadeResult {
	const moduleGraph = cwd ? buildModuleGraph(cwd) : null;
	return computeImpactCascadeImpl(graph, changedFile, moduleGraph);
}

export function formatImpactCascade(result: ImpactCascadeResult, maxFiles?: number): string | undefined {
...
export function recordEntitySnapshotDiff(
	...
	const prev =
		facts.getSessionFact<Map<string, string>>(`${ENTITY_SNAPSHOT_PREFIX}${filePath}`) ?? new Map<string, string>();
	const added: string[] = [];
