...
export function formatImpactCascade(result: ImpactCascadeResult, maxFiles = 4): string | undefined {
	...
	if (result.changedSymbols.length > 0) {
		...
		const suffix =
			result.changedSymbols.length > shown.length ? ` (+${result.changedSymbols.length - shown.length} more)` : "";
