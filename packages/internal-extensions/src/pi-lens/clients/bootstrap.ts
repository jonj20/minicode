...
async function logBootstrapFailures(failures: { name: string; err: unknown }[]): Promise<void> {
	for (const { name, err } of failures) {
		console.error(`[pi-lens] analyzer "${name}" disabled (degraded mode): ${(err as Error)?.message ?? String(err)}`);
	}
	try {
...
export function loadBootstrapClients(): Promise<BootstrapClients> {
	bootstrapPromise ??= (async () => {
		...
		async function loadList<T>(name: string, make: () => Promise<T[]>): Promise<T[]> {
			try {
				return await make();
			} catch (err) {
				failures.push({ name, err });
