...
export function detectProjectLanguageProfile(projectRoot: string, sourceFiles?: string[]): ProjectLanguageProfile {
...
function computeProjectLanguageProfile(projectRoot: string, sourceFiles?: string[]): ProjectLanguageProfile {
	const present = Object.fromEntries(SUPPORTED_FILE_KINDS.map((kind) => [kind, false])) as Record<FileKind, boolean>;
...
export function hasAnyLanguage(profile: ProjectLanguageProfile, kinds: readonly FileKind[]): boolean {
...
export async function detectProjectLanguageProfileAsync(projectRoot: string): Promise<ProjectLanguageProfile> {
	const cached = languageProfileCache.get(projectRoot);
	if (cached) return cached;
