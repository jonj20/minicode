...
function tsLangForFile(filePath: string, kind: string | undefined): string | undefined {
...
function toEntry(
	...
	const complexity =
		typeof metadata.cyclomaticComplexity === "number" ? (metadata.cyclomaticComplexity as number) : undefined;
	const fanout = graph
...
function blastReadArgs(graph: ReviewGraph, normalizedFile: string): { path: string; offset: number; limit: number } {
	...
	const limit = typeof lineCount === "number" && lineCount > 0 ? lineCount : BLAST_RADIUS_DEFAULT_READ_LIMIT;
...
async function computeBlastRadius(
	...
	const files: BlastRadiusFile[] = [...byFile.entries()]
		...
		.sort((a, b) => a.minDepth - b.minDepth || b.dependents - a.dependents || a.file.localeCompare(b.file))
...
function findNearestSymbolName(entries: CallbackOwner[], startLine: number, endLine: number): string | undefined {
...
const ARGUMENT_CONTAINER_NODE_KINDS = new Set(["arguments", "argument_list", "argument_list_expression"]);
...
function ancestorOfType(node: ModuleReportNode, types: Set<string>, maxHops = 6): ModuleReportNode | undefined {
...
function descendantOfType(node: ModuleReportNode, type: string, maxDepth = 2): ModuleReportNode | undefined {
...
function classifyGenericCallback(ctx: CallbackContext): CallbackClassification {
	...
	const include =
		kind !== "callback" || !!propertyName || !!assignedName || flags.some((flag) => flag.startsWith("captures "));
 ...
 */
export async function moduleReport(file: string, cwd: string, options?: ModuleReportOptions): Promise<ModuleReport> {
	...
	const entries = outlineSymbols.map((sym) => toEntry(sym, absPath, normalizedPath, graph, maxRefs, cwd));
 ...
 * that symbol's range.
 */
export async function readSymbol(file: string, symbolName: string, cwd: string): Promise<ReadSymbolResult> {
	const startedAt = Date.now();
	const absPath = path.resolve(cwd, file);
	...
	try {
		callback = extractCallbacks(root, owners, absPath, languageId, callbackWarnings).find(
...
function symbolParentChain(symbol: ExtractedSymbol, symbols: ExtractedSymbol[]): string[] | undefined {
	const chain = symbols
		.filter((candidate) => {
		...
		.sort((a, b) => a.line - b.line || (b.endLine ?? b.line) - b.line - ((a.endLine ?? a.line) - a.line))
