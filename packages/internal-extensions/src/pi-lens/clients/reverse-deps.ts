...
function addEdge(index: ReverseDependencyIndex, fromFile: string, toFile: string) {
	...
	if (from === to) return;
	if (!index.imports[from]) index.imports[from] = [];
	index.imports[from].push(to);
	if (!index.importedBy[to]) index.importedBy[to] = [];
...
export function buildReverseDependencyIndexFromSnapshot(snapshot: ProjectSnapshot): ReverseDependencyIndex | null {
	...
	for (const [filePath, importers] of Object.entries(reverseDeps)) {
		const normalized = normalizeMapKey(filePath);
		index.importedBy[normalized] = sortedUnique([...(index.importedBy[normalized] ?? []), ...importers]);
		index.imports[normalized] ??= [];
