...
export class JscpdClient {
	...
	private hasSourceFilesRecursive(rootDir: string): boolean {
		...
		while (stack.length > 0 && visited < MAX_ENTRIES) {
			...
			for (const entry of entries) {
				...
				if (ignoreMatcher.isIgnored(path.join(dir, entry.name), false)) continue;
				if (
	...
	private async doEnsureAvailable(): Promise<boolean> {
		...
		const localBase = path.join(os.homedir(), ".minicode", ".pi-lens", "tools", "node_modules", ".bin", "jscpd");
	 ...
	 */
	async scan(cwd: string, minLines = 5, minTokens = 50, isTsProject = false): Promise<JscpdResult> {
	...
	private async runScan(cwd: string, minLines: number, minTokens: number, isTsProject: boolean): Promise<JscpdResult> {
		...
		try {
			...
			const { cmd, prefix } = bin ? { cmd: bin, prefix: [] as string[] } : { cmd: "npx", prefix: ["jscpd"] };
