...
import { appendProjectChange, type ProjectChangeRange, type ProjectChangeSource } from "./project-changes.js";
...
const lastAnalyzedStateByFile = new Map<string, { turnIndex: number; stateHash: string }>();
...
function scheduleDebounced(filePath: string, debounceMs: number, deps: ToolResultDeps): Promise<ToolResultReturn> {
...
function singleRange(ranges: Array<{ start: number; end: number }> | undefined): ProjectChangeRange | undefined {
...
> {
	...
	if (event.toolName === "bash" && typeof (event.input as { command?: unknown }).command === "string") {
		...
		const written = extractWrittenPathsFromCommand(command, workspaceRoot).filter(
			(wp) => !isExternalOrVendorFile(wp, workspaceRoot) && !isPathIgnoredByProject(wp, workspaceRoot, false),
		);
	...
	if (deps.readGuard && !getFlag("no-read-guard")) {
		...
		const detailSearchReads = (event.details as { searchReads?: SearchReadLocation[] })?.searchReads;
		if (Array.isArray(detailSearchReads)) searchReads.push(...detailSearchReads);
		if (event.toolName === "bash" && typeof (event.input as { command?: unknown }).command === "string") {
			const command = (event.input as { command: string }).command;
			const output = event.content.map((part) => (typeof part.text === "string" ? part.text : "")).join("\n");
	...
	if (lastAnalyzed?.turnIndex === runtime.turnIndex && lastAnalyzed.stateHash === initialStateHash) {
	...
	if (!result.isError && !getFlag("no-autoformat") && !getFlag("immediate-format") && nodeFs.existsSync(filePath)) {
