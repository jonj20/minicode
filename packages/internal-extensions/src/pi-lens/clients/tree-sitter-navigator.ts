...
export class TreeSitterNavigator {
	...
	isInAsyncContext(node: ASTNode): boolean {
		...
		const functionTypes = ["function_declaration", "function_expression", "arrow_function", "method_definition"];
	...
	isShadowed(node: ASTNode, varName: string): boolean {
		...
		while (current) {
			...
			if (
				["function_declaration", "function_expression", "arrow_function", "method_definition"].includes(
					current.type,
				)
			) {
	...
	getScopeContext(node: ASTNode): ScopeContext {
		...
		const functionDepth = ancestors.filter((a) =>
			["function_declaration", "function_expression", "arrow_function", "method_definition"].includes(a.type),
		).length;
