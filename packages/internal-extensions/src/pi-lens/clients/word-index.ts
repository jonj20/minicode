 ...
 */
export function buildWordIndex(files: Array<{ path: string; content: string }>): WordIndex {
 ...
 */
export function searchWordIndex(index: WordIndex, query: string, options: RankOptions = {}): RankedFile[] {
...
export function deserializeWordIndex(data: SerializedWordIndex | null | undefined): WordIndex | null {
	if (!data || !Array.isArray(data.files) || !Array.isArray(data.postings) || !Array.isArray(data.docLengths)) {
		return null;
	}
	const docLengths = new Map<string, number>();
	for (let i = 0; i < data.files.length; i++) {
		docLengths.set(data.files[i], data.docLengths[i] ?? 0);
	}
