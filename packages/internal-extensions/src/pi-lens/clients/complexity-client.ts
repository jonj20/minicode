...
const GO: LangNodes = {
	...
	decision: new Set(["if_statement", "for_statement", "expression_case", "type_case", "communication_case"]),
	cognitive: new Set([
		"if_statement",
		"for_statement",
		"expression_switch_statement",
...
const RUST: LangNodes = {
	...
	decision: new Set(["if_expression", "while_expression", "for_expression", "loop_expression", "match_arm"]),
	cognitive: new Set([
		"if_expression",
		"while_expression",
		"for_expression",
...
function calculateMaxParams(functions: FunctionMetrics[]): number {
	...
	return Math.min(
		10,
		Math.max(2, Math.round(functions.reduce((a, f) => a + f.length, 0) / Math.max(1, functions.length) / 5)),
	);
}
...
export class ComplexityClient {
	...
	constructor(verbose = false) {
		this.log = verbose ? (msg: string) => console.error(`[complexity] ${msg}`) : () => {};
	}
	...
	private computeMetrics(absolutePath: string, content: string, root: TsNode, nodes: LangNodes): FileComplexity {
		...
		return {
			...
			maintainabilityIndex:
				Math.round(calculateMaintainabilityIndex(funcStats.avgCyclomatic, codeLines, commentLines) * 10) / 10,
