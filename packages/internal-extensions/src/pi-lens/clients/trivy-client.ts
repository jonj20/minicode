...
export function isTrivyEnabled(cwd: string): boolean {
	try {
		...
		const trivy = (config.raw as { trivy?: { enabled?: unknown } } | undefined)?.trivy;
...
export function resolveSeverityFloor(cwd: string): TrivySeverity[] {
	...
	try {
		const config = loadPiLensProjectConfig(cwd);
		raw = (config.raw as { trivy?: { minSeverity?: unknown } } | undefined)?.trivy?.minSeverity;
	} catch {
		raw = undefined;
	}
	...
	return (["LOW", "MEDIUM", "HIGH", "CRITICAL"] as TrivySeverity[]).filter((s) => SEVERITY_RANK[s] >= floor);
}
	...
	for (const resultEntry of results) {
		...
		for (const vulnEntry of vulns) {
			...
			findings.push({
				...
				fixedVersion: typeof v.FixedVersion === "string" && v.FixedVersion.trim() ? v.FixedVersion : undefined,
