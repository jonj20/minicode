...
export function normalizeOpengrepConfigArg(config: string | undefined, cwd: string): string | undefined {
...
export function resolveOpengrepConfig(
	...
	const flagConfig = typeof flags?.config === "string" && flags.config.trim() ? flags.config.trim() : undefined;
