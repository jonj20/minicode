...
export interface KnipIssue {
	type: "export" | "file" | "dependency" | "devDependency" | "unlisted" | "bin" | "enumMember";
	name: string;
	file?: string;
	line?: number;
	package?: string;
...
export class KnipClient {
	...
	constructor(verbose = false) {
		this.log = verbose ? (msg: string) => console.error(`[knip] ${msg}`) : () => {};
	}
	...
	private resolveProjectRoot(startDir: string, homeDirOverride?: string): string | null {
		const markers = ["package.json", "knip.json", "knip.ts", "knip.config.js", "knip.config.ts"];
	...
	async findUnusedExports(filePath: string): Promise<string[]> {
		...
		return result.unusedExports.filter((e) => e.file?.includes(basename)).map((e) => e.name);
	}
	...
	private parseOutput(output: string): KnipResult {
		try {
			...
			for (const entry of fileEntries) {
				...
				const push = (arr: any[], type: KnipIssue["type"], _target: KnipIssue[]) => {
