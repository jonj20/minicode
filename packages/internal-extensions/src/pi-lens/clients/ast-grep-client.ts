...
import type { AstGrepDiagnostic, AstGrepMatch, RuleDescription, SgMatch } from "./ast-grep-types.js";
...
export class AstGrepClient {
	...
	constructor(ruleDir?: string, verbose = false) {
		...
		this.ruleDir =
			ruleDir || (fs.existsSync(projectRuleDir) ? projectRuleDir : resolvePackagePath(import.meta.url, "rules"));
		this.log = verbose ? (msg: string) => console.error(`[ast-grep] ${msg}`) : () => {};
	...
	async validateRule(ruleYaml: string): Promise<{ valid: boolean; error?: string }> {
...
`;
	...
	formatMatches(matches: AstGrepMatch[], isDryRun = false, showModeIndicator = false): string {
		return this.runner.formatMatches(matches as SgMatch[], isDryRun, 50, showModeIndicator);
	}
