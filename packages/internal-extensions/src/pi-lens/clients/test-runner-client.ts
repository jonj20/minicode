...
export class TestRunnerClient {
	...
	constructor(verbose = false) {
		this.log = verbose ? (msg: string) => console.error(`[test-runner] ${msg}`) : () => {};
	}
	 ...
	 */
	detectRunner(cwd: string, sourceFilePath?: string): { runner: string; config: RunnerConfig } | null {
		...
		for (let i = 0; i < patterns.testExts.length; i++) {
			...
			if (testExt.includes("*")) {
				...
				const match = files.find(
					(f) => f === pattern || (f.startsWith("test_") && f.endsWith(".py") && f.includes(basename)),
				);
				if (match) {
	 ...
	 * tests run.
	 */
	async runTestFileAsync(testFile: string, cwd: string, runner: string, config: RunnerConfig): Promise<TestResult> {
					...
					for (const test of suite.assertionResults) {
						if (test.status === "failed") {
							failures.push({
								...
								location: test.location ? `${path.relative(cwd, testFile)}:${test.location.line}` : undefined,
	...
	private parsePytestOutput(
		...
		let failureMatch = failureRegex.exec(output);
		while (failureMatch !== null) {
			failures.push({
				name: failureMatch[1],
				message: failureMatch[2].trim().slice(0, 500),
				location: failureMatch[1].replace("::", ":"),
			});
			failureMatch = failureRegex.exec(output);
		}

		// Also look for assertion errors with traceback
		const tracebackRegex = /_{10,}\s*\n\s*(\w+Error:\s*.+?)(?:\n|$)/gs;
		let tracebackMatch = tracebackRegex.exec(output);
		while (tracebackMatch !== null) {
			// Add to last failure if exists, or create generic
			if (failures.length > 0 && !failures[failures.length - 1].stack) {
				failures[failures.length - 1].stack = tracebackMatch[1].trim().slice(0, 1000);
			}
			tracebackMatch = tracebackRegex.exec(output);
		}
	...
	private findTestFileByImport(sourceFilePath: string, cwd: string): string | null {
		...
		const searchDirs = [path.join(cwd, "tests"), path.join(cwd, "__tests__"), path.dirname(sourceFilePath)];
