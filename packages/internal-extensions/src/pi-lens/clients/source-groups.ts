...
export function partitionSourceFiles(sourceRoot: string, files: string[], maxFiles = 12): SourceGroup[] {
	if (maxFiles <= 0) {
...
function partitionAt(sourceRoot: string, files: string[], maxFiles: number, depth: number): SourceGroup[] {
...
function currentLabel(sourceRoot: string, files: string[], depth: number): string {
	...
	const relativePath = first.startsWith(`${sourceRoot}/`) ? first.slice(sourceRoot.length + 1) : first;
