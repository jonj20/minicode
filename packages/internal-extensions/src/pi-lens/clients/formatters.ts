...
export async function tryLazyInstallFormatterTool(tool: "rubocop" | "rustfmt", cwd: string): Promise<boolean> {
...
function hasExplicitFormatterConfig(formatterName: string, cwd: string): boolean {
...
export const biomeFormatter: FormatterInfo = {
	...
	async resolveCommand(filePath, cwd) {
		...
		if (global) return [global, "format", "--write", ...editorConfigFlag, filePath];
...
export const rubocopFormatter: FormatterInfo = {
	...
	async resolveCommand(filePath, cwd) {
		if (await canUseBundleExec(cwd)) return ["bundle", "exec", "rubocop", "-a", "--no-color", filePath];
		return null;
	},
	async detect(cwd: string) {
...
export async function getFormattersForFile(filePath: string, cwd: string): Promise<FormatterInfo[]> {
...
export async function formatFile(filePath: string, formatter: FormatterInfo): Promise<FormatterResult> {
	try {
