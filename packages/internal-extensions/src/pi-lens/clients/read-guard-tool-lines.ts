...
function formatSimilarLines(suggestions: Array<{ line: number; text: string }>): string {
	...
	if (filePath) {
		logReadGuardEvent({
			...
			metadata: {
				tool: "edit",
				source:
					ranges.length === 1 && ranges[0][0] === ranges[0][1] ? "hashline_set_line" : "hashline_replace_lines",
...
function exactOldTextForApply(rawContentLf: string, oldText: string, candidate: string): string | undefined {
	...
	for (let i = 0; i < edits.length; i++) {
		...
		if (occurrenceLines.length === 0) {
			...
			if (quoteHit !== undefined) {
				...
				const offsetHint = lineHint !== undefined ? `\`offset=${Math.max(1, lineHint - 2)} limit=20\`` : undefined;
 ...
 */
export function tryCorrectIndentationMismatchFromContent(oldText: string, content: string): string | undefined {
...
export function tryCorrectIndentationMismatch(oldText: string, filePath: string): string | undefined {
	try {
 ...
 */
function findUnicodePunctuationInsensitiveCandidate(content: string, oldText: string): string | undefined {
 ...
 */
export function relocateEditRange(input: unknown, from: [number, number], to: [number, number]): boolean {
...
export function getTouchedLinesForGuard(event: unknown, filePath?: string, sessionId?: string): GuardLineResult {
	if (isToolCallEventType("edit", event as any)) {
		...
		const editInput = (event as { input?: unknown }).input as Partial<Pick<EditToolInput, "path">> & {
			oldRange?: { start: { line: number }; end: { line: number } };
			edits?: Array<
		...
		const hashlineResult = resolveHashlineEditInput(editInput as Record<string, unknown>, filePath, sessionId);
		if (hashlineResult) return hashlineResult;
		if (editInput.oldRange) {
			const touchedLines: [number, number] = [editInput.oldRange.start.line, editInput.oldRange.end.line];
			if (filePath) {
				logReadGuardEvent({
					event: "touched_lines_detected",
					sessionId,
