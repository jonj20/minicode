...
function lspUnavailableMessage(filePath: string, health: LspHealthLike | undefined): string | undefined {
	...
	const reason =
		(health.serverCountAttempted ?? 0) === 0 ? "no LSP server configured" : "no LSP client is currently ready";
	const stale =
...
export function createLspDiagnosticsTool() {
	return {
		...
		renderResult: compactRenderResult<{
			...
			const scope = typeof files === "number" && files > 1 ? ` across ${files} files` : target ? ` ${target}` : "";
...
async function collectDiagnosticsForFile(
	...
	try {
		...
		if (waitMs !== undefined && typeof serviceWithTouch.touchFile === "function") {
...
async function runBatchFileDiagnostics(
	...
	const results = await mapWithConcurrency(
		absPaths,
		options.concurrency,
		(file) => collectFileDiagnosticResult(file, severity, lspService, options.waitMs),
		options.signal,
		options.onProgress,
	);
...
async function runDirectoryDiagnostics(
	...
	if (total === 0) {
		...
		for (const d of display) {
			...
			const loc = d.line !== undefined ? `${relPath}:${d.line + 1}:${(d.character ?? 0) + 1}` : d.file;
			const src = d.source ? `[${d.source}]` : "";
			const code = d.code ? ` (${d.code})` : "";
			lines.push(`${loc}: ${sevName}${src}${code}: ${d.message}`);
		}
