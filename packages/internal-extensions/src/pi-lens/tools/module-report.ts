...
import { moduleReport, readEnclosing, readSymbol } from "../clients/module-report.js";
...
export function createReadEnclosingTool(getProjectRoot: () => string, recordSymbolRead: ReadRecorder) {
	return {
		...
		renderResult: compactRenderResult<{
			...
			const range = details?.startLine && details?.endLine ? `:${details.startLine}-${details.endLine}` : "";
