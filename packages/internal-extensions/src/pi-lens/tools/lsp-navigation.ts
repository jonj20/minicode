...
function emptyReasonForOperation(operation: LspNavigationOperation): string {
	...
	if (operation === "incomingCalls" || operation === "outgoingCalls") return "no-call-hierarchy-results";
	return "no-results";
}
...
function searchReadFromUriRange(uri: unknown, range: unknown): SearchReadLocation | undefined {
	...
	try {
		return {
			...
			endLine:
				typeof endLine === "number" && Number.isFinite(endLine) ? Math.max(1, Math.floor(endLine) + 1) : undefined,
		};
	} catch {
		return undefined;
	}
...
function collectSearchReadsForOperation(
	...
	if (["definition", "typeDefinition", "declaration", "references", "implementation"].includes(operation)) {
...
function formatCapabilities(snapshots: CapabilitySnapshot[], filePath?: string): string {
	...
	const rows: Array<[string, (snapshot: CapabilitySnapshot) => boolean, string?]> = [
		...
		["workspaceDiagnostics", (s) => s.workspaceDiagnosticsSupport.mode === "pull", "pull diagnostics"],
...
export function createLspNavigationTool(getFlag: (name: string) => boolean | string | undefined) {
	return {
		...
		async execute(
			...
			const filePath = rawPath ? (path.isAbsolute(rawPath) ? rawPath : path.resolve(ctx.cwd || ".", rawPath)) : "";
			...
			if (operation === "workspaceDiagnostics") {
				...
				const note =
					noteMap2[diagnosticsMode] ?? "Note: workspace diagnostics mode unknown (no active capability snapshot)";
				return finalize(
					{
						content: [
							{
			...
			const runWorkspaceSymbolOperation = async (): Promise<SymbolNode[]> => {
				...
				try {
					...
					const filtered = (Array.isArray(raw) ? raw : [raw]).filter(
						(s) => typeof s === "object" && s !== null && (!s.kind || NAVIGABLE_SYMBOL_KINDS.has(s.kind)),
					) as SymbolNode[];
			...
			return finalize(
				{
					...
					details: {
						...
						columnResolution: columnResolution?.strategy === "explicit" ? undefined : columnResolution,
						resultCount,
					},
				},
				{
