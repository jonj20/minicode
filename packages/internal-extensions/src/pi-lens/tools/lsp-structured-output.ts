...
function lspToolLocationFromFileRange(filePath: string, range: unknown): LspToolLocation | undefined {
...
export function collectLspToolLocationsForOperation(
	...
	if (
		["definition", "typeDefinition", "declaration", "references", "implementation", "prepareCallHierarchy"].includes(
			operation,
		)
	) {
...
export function buildLspNavigationEnvelope(options: BuildLspNavigationEnvelopeOptions): LspToolEnvelope {
