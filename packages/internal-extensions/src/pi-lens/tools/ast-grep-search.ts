...
function formatGroupedByFile(matches: AstGrepMatch[]): string {
	...
	const lines = [`${fileCount} file${fileCount === 1 ? "" : "s"}, ${total} match${total === 1 ? "" : "es"}:`];
...
function toLineSpan(match: AstGrepMatch): { file: string; startLine: number; endLine: number } | null {
...
function toMatchLocations(matches: AstGrepMatch[], contextLines: number | undefined): MatchLocation[] {
	const margin =
		...
		async execute(
			...
			try {
				...
				const PAGE_SIZE = Math.max(
					...
					Math.min(
						MAX_PAGE_SIZE,
						Number.isFinite(maxMatches as number) ? Math.floor(maxMatches as number) : DEFAULT_PAGE_SIZE,
					),
				);
				...
				const output = groupByFile ? formatGroupedByFile(page) : astGrepClient.formatMatches(page);
				const hint = page.length === 0 && !result.error ? getPatternHint(pattern, lang, selector) : undefined;
				const paginationNote =
					hasMore && page.length > 0
