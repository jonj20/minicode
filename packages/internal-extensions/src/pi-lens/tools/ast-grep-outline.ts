...
import type { AstGrepClient, AstGrepOutlineFile, AstGrepOutlineItem } from "../clients/ast-grep-client.js";
...
export function createAstGrepOutlineTool(astGrepClient: AstGrepClient) {
	return {
		...
		async execute(
			...
			const cwd = ctx.cwd || ".";
			const rawPaths = (params.paths ?? []).filter((p): p is string => typeof p === "string" && p.trim().length > 0);
