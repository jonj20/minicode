...
function appendProjectDiagnosticsDeltaLines(
	...
	const diagnostics = (report?.diagnostics ?? []).filter(
		(diagnostic) =>
			includeFile(diagnostic.filePath) && matchesSeverity(projectDiagnosticToWidget(diagnostic), severity),
	);
...
function formatDeltaMode(
	...
	const actionableEntry = cacheManager.readCache<ActionableWarningsReport>("actionable-warnings", cwd);
	const qualityEntry = cacheManager.readCache<CodeQualityWarningsReport>("code-quality-warnings", cwd);
...
async function formatFullMode(
	...
	if (aborted) {
		const timedOut =
			(signal as (AbortSignal & { reason?: { name?: string } }) | undefined)?.reason?.name === "TimeoutError";
		const note = timedOut
