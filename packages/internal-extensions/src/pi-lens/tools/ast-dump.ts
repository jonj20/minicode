...
function createAstDumpToolWithName(astGrepClient: AstGrepClient, name: "ast_dump" | "ast_grep_dump") {
	...
	return {
		...
		renderResult: compactRenderResult<{ lang?: string }>(({ details, args, isError, lineCount, text }) => {
		...
		async execute(_toolCallId: string, params: Record<string, unknown>, _signal: AbortSignal, _onUpdate: unknown) {
