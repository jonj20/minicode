...
const cfg: typeof DEFAULTS & Record<string, number | string> = (() => {
	...
	const merged = { ...DEFAULTS, ...(fromFile ?? {}) } as typeof DEFAULTS & Record<string, number | string>;
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("message_end", (event, _ctx) => {
		...
		if (thinkingAborted) {
			...
			pi.sendMessage({ customType: "loop-police", content: advice, display: true }, { triggerTurn: true });
