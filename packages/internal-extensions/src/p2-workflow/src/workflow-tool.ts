...
const workflowToolSchema = Type.Object({
	...
	preset: Type.Optional(
		Type.String({
			description: `Run a built-in preset workflow by name instead of a custom script. Available: ${PRESET_NAMES.join(", ")}.`,
		}),
	),
	args: Type.Optional(
...
function normalizeWorkflowToolArgs(args: unknown): WorkflowToolInput {
	if (!args || typeof args !== "object")
		throw new Error("workflow requires an object argument with a script or preset");
	const value = args as Record<string, unknown>;
	if (typeof value.preset === "string") {
		const script = PRESETS[value.preset];
		if (!script) {
			throw new Error(`Unknown preset "${value.preset}". Available: ${PRESET_NAMES.join(", ")}`);
		}
		return { preset: value.preset, script: normalizeWorkflowScript(script), args: value.args };
	}
	if (typeof value.script !== "string") {
		throw new Error("workflow requires either `script` or `preset` parameter");
	}
