...
export function renderUsageSummary(buckets: UsageBuckets, theme: Theme): string[] {
	...
	const percentStr = buckets.percent !== null ? `${Math.round(buckets.percent)}%` : "?%";
	const pct = (n: number) => (buckets.contextWindow > 0 ? `${Math.round((n / buckets.contextWindow) * 100)}%` : "0%");
	...
	return [
		...
		"",
		`${theme.fg("accent", SYM_SYSTEM)} System Prompt: ${fmtTokens(buckets.systemPromptTokens).padStart(
			7,
		)} (${pct(buckets.systemPromptTokens)})`,
		`${theme.fg("muted", SYM_TOOL)} Tools:          ${fmtTokens(buckets.toolTokens).padStart(
			7,
		)} (${pct(buckets.toolTokens)})`,
		`${theme.fg("success", SYM_MESSAGE)} Messages:      ${fmtTokens(buckets.messageTokens).padStart(
			7,
		)} (${pct(buckets.messageTokens)})`,
		`${theme.fg("dim", SYM_FREE)} Empty:          ${fmtTokens(buckets.freeTokens).padStart(
			7,
		)} (${pct(buckets.freeTokens)})`,
		`${theme.fg("warning", SYM_BUFFER)} Buffer:         ${fmtTokens(buckets.bufferTokens).padStart(
			7,
		)} (${pct(buckets.bufferTokens)})`,
	];
}
