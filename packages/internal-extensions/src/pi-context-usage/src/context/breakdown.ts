...
export type TurnMessageDetail = {
	id: string;
	role: "user" | "assistant" | "toolResult" | "bashExecution" | "custom" | "compactionSummary";
	label: string;
	preview: string;
	tokens: number;
	chars: number;
...
function determineDominantRole(details: TurnMessageDetail[]): TurnBreakdown["dominantRole"] {
...
export function formatSystemToolsSection(section: SystemToolsSection, theme: Theme): string[] {
	const nameWidth = Math.max("Tool".length, ...section.tools.map((tool) => tool.name.length), "System prompt".length);
...
export function formatConversationSection(turns: TurnBreakdown[], theme: Theme, maxRows = turns.length): string[] {
