...
function buildSummary(pi: ExtensionAPI, ctx: ExtensionCommandContext): UsageBuckets | null {
	...
	return computeUsageBuckets(usage, ctx.sessionManager.getBranch(), ctx.model, systemPromptTokens, toolTokens);
}

function notifySummary(ctx: ExtensionCommandContext, buckets: UsageBuckets, _pi?: ExtensionAPI): void {
...
function wrapInBox(lines: string[], width: number, colorFn: (s: string) => string, title?: string): string[] {
...
class ContextDetailsOverlay implements Component {
	...
	private getVisibleRows(): VisibleRow[] {
		...
		if (this.sectionExpanded.systemPrompt) {
			...
			if (this.systemTools.cachedTokens !== null && this.systemTools.cachedTokens !== this.systemTools.totalTokens) {
	...
	private moveToSection(direction: 1 | -1): void {
		...
		let currentSection = sectionIndices.indexOf(this.focusIndex);
		if (currentSection === -1) {
...
async function showDetails(pi: ExtensionAPI, ctx: ExtensionCommandContext): Promise<void> {
	...
	await ctx.ui.custom<void>(
		(tui, theme, _keybindings, done) => new ContextDetailsOverlay(tui, theme, buckets, systemTools, turns, done),
		{
			overlay: true,
			overlayOptions: {
				width: "90%",
