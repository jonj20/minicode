import type { ThinkingLevel as AiThinkingLevel, AssistantMessage, Message, UserMessage } from "@earendil-works/pi-ai";
import {
	type AgentSession,
...
function parseBtwThinkingArgs(
	args: string,
): { action: "show" } | { action: "clear" } | { action: "set"; thinkingLevel: SessionThinkingLevel } {
	const trimmed = args.trim();
	if (!trimmed) {
		return { action: "show" };
	}
...
function appendTranscriptEntry<T extends BtwTranscriptEntry>(state: BtwTranscriptState, entry: Omit<T, "id">): T {
...
function finishTranscriptTurn(state: BtwTranscriptState, turnId?: number | null): void {
	...
	if (!hasEndBoundary) {
		appendTranscriptEntry(state, { type: "turn-boundary", turnId: resolvedTurnId, phase: "end" } as Omit<
			Extract<BtwTranscriptEntry, { type: "turn-boundary" }>,
			"id"
		>);
	}
...
export default function (pi: ExtensionAPI) {
	...
	function handleBtwSessionEvent(
		...
		if (event.type === "tool_execution_end") {
			setOverlayStatus(
				sessionRuntime.session.isStreaming ? `⏳ running tool: ${event.toolName}` : "⏳ streaming...",
				ctx,
			);
			return;
		}
	...
	async function resolveBtwModel(ctx: ExtensionCommandContext, notifyOnFallback = false): Promise<ResolvedBtwModel> {
		if (btwModelOverride) {
