...
| 13 | **pi-continue** | 会话继续 | - | `/continue` | - | 仅 session entries |
| 14 | **pi-fff** | 模糊文件和内容搜索 | `ffgrep`, `fffind`, `fff-multi-grep` | `/fff-mode`, `/fff-health`, `/fff-rescan` | - | 仅内存 |
| 15 | **pi-hermes-memory** | 持久化记忆：长期记忆、每日日志 | `memory`, `memory_search`, `session_search`, `skill_manage` | `/memory-insights`, `/memory-skills`, `/memory-consolidate` 等7个 | - | `~/.pi/agent/memory/` |
| 16 | **pi-lens** | 上下文透镜：LSP/AST分析 | `ast-dump`, `lsp-diagnostics`, `lsp-navigation` 等11个 | `/lens-toggle`, `/lens-context-toggle` 等7个 | - | 仅内存 |
| 17 | **pi-loop-police** | 循环检测防护：防止无限循环 | - | `/loop-police` | - | `loop-police.json` |
| 18 | **pi-mcp-adapter** | MCP 适配器：统一接入 MCP 工具 | `mcp`, `<server>_<tool>` | `/mcp`, `/mcp-auth` | - | 仅内存 |
| 19 | **pi-rewind** | 会话回退：回退到之前的会话状态 | - | `/rewind` | `escape escape` | 仅 session entries |
| 20 | **pi-subagents** | 子代理委托：single/parallel/chain三种模式，最多8并发 | `Agent`, `StopAgent`, `AgentStatus` | `/agents` | - | `~/.pi/agent/agents/*.md` |
| 21 | **pi-tasks** | 轻量任务分解：JSONL持久化，支持子任务和自动执行 | `TaskCreate`, `TaskList`, `TaskGet`, `TaskUpdate`, `TaskOutput`, `TaskStop`, `TaskExecute` | `/tasks` | - | `~/.pi/agent/tasks/tasks.jsonl` |
...
- [TOOLS-AND-COMMANDS.md](./TOOLS-AND-COMMANDS.md) — 工具与命令完整清单
